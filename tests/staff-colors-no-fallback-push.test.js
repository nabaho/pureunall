/* ══ 【4걸음 알림 — 지운다】 ════════════════════════════════════
   이알피 법인 대시보드가 data/staff_colors 를 올리는 규칙을 본다.
   4걸음에서 그 화면이 사라지면 올리는 쪽도 함께 사라진다.
   승계: tests/cal-fills-staff-colors.test.js — 푸른 캘린더는 «빠진 사람만» 채운다
         (통째로 다시 올리지 않으므로 여기 여섯 규칙 중 ②④-2 는 애초에 일어나지 않는다).

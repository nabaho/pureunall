...
test('★ 연습은 「시험 발송」으로 — 연습 채우기가 그것을 «말한다»', () => {
  /* ⚠ 「N곳에 보내기」는 진짜 자문사 110곳으로 나간다. 연습이 거기로 가면 사고다.
     그래서 연습 채우기 확인창이 반드시 그 말을 해야 한다.
   ★ 2026-09-13 에 단추 이름이 「나에게 시험 발송」 → 「시험 발송」으로 바뀌었다
     (대표 지시 — 이제 받을 주소를 골라 넣는다). 가리키는 이름이 어긋나면
     화면이 없는 단추를 누르라고 말하게 된다. 그래서 이름도 함께 못 박는다. */
  ...
  assert.match(fn, /「시험 발송」/, '연습을 어디로 보내야 하는지 안 말합니다');
  assert.ok(fn.indexOf('나에게 시험 발송') < 0,
    '★ 없는 단추 이름(「나에게 시험 발송」)을 누르라고 말합니다');
...
test('★ 연락처가 «옆 열»에 있고 머리와 몸통의 열 수가 맞는다', () => {
  ...
  /* ⚠ 2026-09-09 부터 표가 «셋»이었다 — 담당자 · 대표자 · 따로 더한 분.
     ★ 2026-09-13 대표 지시 「사업장의 대표와 담당자를 한줄에 넣고」로
       앞의 둘이 «사업장 표 하나»로 합쳐졌다. 그 표는 누구 칩에 따라 열이 늘고 줄어
       («${대표볼까 ? '<th>…' : ''}») 칸을 세어서는 맞춰 볼 수 없다.
       대신 «머리와 몸통이 같은 조건을 쓰는가»를 본다 — 조건이 어긋나면 밀린다.
     ⚠ 칸을 세는 잣대는 열이 «고정된» 표에만 쓴다. 섞어 쓰면 둘 다 헐거워진다. */
  const 표들 = [...몸.matchAll(/<table class="list"><tr>([\s\S]*?)<\/tr>\$\{([^${}]+)\}<\/table>/g)];
  assert.ok(표들.length >= 2, '갈래별 표를 못 찾았습니다 (찾은 것 ' + 표들.length + '개)');

  표들.filter(function(m){ return m[2].trim() !== '집표'; }).forEach(function(m){
    const 이름 = m[2];
    /* 머리 — ${머리체크} 는 <th> 하나다 */
  ...
  /* ★ 사업장 표 — 머리와 몸통이 «같은 조건»으로 열을 켜고 끈다.
       ⚠ 하나만 고치면 표가 통째로 한 칸씩 밀린다. 그때 담당자 자리에 대표자가 앉는다. */
  const 머리 = (표들.find(function(m){ return m[2].trim() === '집표'; }) || [])[1];
  assert.ok(머리, '사업장 표(집표)를 못 찾았습니다');
  ['대표볼까', '담당볼까'].forEach(function(조건){
    assert.ok(머리.indexOf(조건) >= 0, '사업장 표 머리가 ' + 조건 + ' 를 안 봅니다');
    const 시작 = 몸.indexOf('const 집표 =');
    assert.ok(시작 >= 0, '집표 를 만드는 곳을 찾을 수 없습니다');
    const 줄 = 몸.slice(시작, 몸.indexOf("}).join('')", 시작));
    assert.ok(줄.indexOf(조건) >= 0,
      '사업장 표 줄이 ' + 조건 + ' 를 안 봅니다 — 머리와 몸통의 열 수가 어긋납니다');
  });

  /* 연락처가 «아래»가 아니라 «옆»인지 (대표 지시 2026-09-03)
     ★ 2026-09-13 한 줄로 합치면서 연락처 «열»은 사라졌다 — 한 집에 담당자가
       여럿일 수 있어 열로는 세로를 맞출 수 없다. 이름 «옆»에 둔다는 뜻은 지킨다.
     ⚠ 그래도 tel 글꼴은 쓴다 — 숫자 폭이 흔들리면 옆에 두어도 읽기 어렵다. */
  assert.ok(몸.indexOf('전화칸(x.전화)') >= 0,
    '연락처를 tel 글꼴로 안 그립니다 — 숫자 폭이 흔들립니다');
  assert.match(몸, /class="tel"/, '연락처 글꼴 자리가 없습니다');

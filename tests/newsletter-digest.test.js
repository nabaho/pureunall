...
/* ⚠⚠ 2026-09-12 에 «뒤집힌» 규칙이다. 처음에는 꼭지 끝에 「N건 자세히 보기」 하나를
     두었다 — 링크가 많으면 눈이 쉴 곳이 없다고 보아서다. 대표께서 그날 바로
     「각각의 건에 대해 클릭하면 팝업으로 … 몇건 자세히보기 이렇게 안본다」고 하셨다.
     읽다가 궁금한 «그 줄»을 누르는 것이 사람의 손이다. 옛 메모를 읽고 되돌리지 말 것. */

test('★★ 요약의 줄«마다» 제 건으로 간다 — 꼭지 하나짜리 링크로 되돌리지 말 것', () => {
  const 요 = 짓기(회차(), 설정(), { 요약: true, 지역: '전국' });
  assert.ok(!요.서식.includes('건 자세히 보기'),
    '꼭지 하나짜리 「N건 자세히 보기」가 돌아왔다 — 대표께서 그렇게는 안 보신다');
  /* 미리보기가 아니면 추적으로 감싸이므로 감싸기 전 목록(링크들)으로 본다 */
  const 건 = 요.링크들.filter((u) => u.indexOf('#n-') > 0);
  assert.equal(건.length, 4,
    '줄마다 제 건으로 가지 않는다 — 기사1·자료1·판례1·우리글1 이라야 한다 (찾은 것 ' + 건.length + ')');
  assert.ok(!요.링크들.some((u) => u.indexOf('#g-') > 0),
    '꼭지로만 가는 옛 링크가 남았다');
});

test('★★ 요약이 가리키는 자리가 전문에 «실제로» 있다', () => {
  /* 자리표가 어긋나면 눌러도 아무 일이 안 난다 — 그런데 겉으로는 멀쩡해 보인다.
     ⚠ 전문은 갈래(자료·판례·기사)로 «갈라» 그리므로, 번호를 그린 차례로 매기면
       요약의 셋째 줄이 전문의 첫 칸을 가리키게 된다. 꼭지 안의 차례로 매겨야 한다. */
  const d = 큰회차();
  const 요 = 짓기(d, 설정(), { 요약: true, 지역: '전국' });
  const 전 = 짓기(d, 설정(), { 미리보기: true, 요약: false, 지역: '전국' });
  const 있는자리 = new Set((전.서식.match(/id="(n-[^"]+)"/g) || [])
    .map((s) => s.slice(4, -1)));
  const 가리킴 = 요.링크들.filter((u) => u.indexOf('#n-') > 0)
    .map((u) => u.slice(u.indexOf('#n-') + 1));
  assert.ok(가리킴.length >= 12, '가리키는 곳이 너무 적다 (' + 가리킴.length + ')');
  가리킴.forEach((k) => {
    assert.ok(있는자리.has(k), '요약이 ' + k + ' 를 가리키는데 전문에 그 자리가 없다');
  });
});

test('★★ 자리표 «n-꼭지-i» 에 정말 그 i 번째 건이 있다 — 한 칸만 밀려도 딴 것이 열린다', () => {
  /* ⚠ 위 검사(가리키는 곳이 다 있다)만으로는 «한 칸 밀림»을 못 잡는다 —
       요약과 전문이 같은 번호 매김을 쓰므로 둘이 나란히 밀리면 그대로 맞아떨어진다.
...
test('자리표를 «원본에» 붙이지 않는다 — 붙이면 회차와 함께 서버에 저장된다', () => {
  const d = 회차();
  짓기(d, 설정(), { 요약: true, 지역: '전국' });
  (d.안.news || []).forEach((x) => {
    assert.ok(!('_자리' in x), '원본 자료에 _자리 가 생겼다 — 없던 칸이 서버로 새 나간다');
  });
});
...
test('★ 링크는 모두 «그 회차의» 웹 쪽으로 간다 — 맨 위 「전문 보기」도 있다', () => {
  const 요 = 짓기(회차(), 설정(), { 요약: true, 지역: '전국' });
  ...
  것.forEach((u) => {
    assert.ok(u.indexOf('newsView?i=2026-09-w1') > 0, '엉뚱한 회차로 가는 링크가 있다: ' + u);
  });
});
...
/* ══════ ⑥-2 한 건만 띄우는 창 (대표 지시 2026-09-12) ══════ */

test('★★ 웹 쪽이 «한 건만 띄우는 창»을 갖는다', () => {
  const 쪽 = NV.쪽('제목', '<div id="n-news-0" data-pop="1">가</div>');
  assert.ok(쪽.includes('id="pop"'), '창이 없다');
  assert.ok(쪽.includes('id="popb"'), '창 안에 넣을 자리가 없다');
  assert.ok(쪽.includes('id="popx"'), '닫기 단추가 없다');
...
test('★ 창 스크립트가 «돌아가는 글»이다 — 깨진 채 나가면 아무 건도 안 열린다', () => {
  ...
  assert.doesNotThrow(() => new Function(m[1]), '창 스크립트에 구문 오류가 있다');
});

test('★★ 창은 내용을 «다시 짓지 않고» 그 칸을 베낀다 — 편지와 다를 수가 없게', () => {
  const 쪽 = NV.쪽('제목', '<div id="n-news-0" data-pop="1">가</div>');
  const 글 = /<script>([\s\S]*?)<\/script>/.exec(쪽)[1];
  assert.match(글, /cloneNode\(true\)/, '칸을 베끼지 않는다 — 내용을 새로 지으면 편지와 어긋난다');
});

test('★★ 칸 «안의 링크»는 그대로 통한다 — 안 그러면 내려받기를 누를 길이 없다', () => {
  const 글 = /<script>([\s\S]*?)<\/script>/.exec(NV.쪽('ㄱ', 'ㄴ'))[1];
  assert.match(글, /closest\("a"\)\)return/,
    '링크를 눌러도 창이 열린다 — 원문·내려받기로 갈 길이 막힌다');
});
...
test('닫으면 주소의 자리표도 지운다 — 안 지우면 새로 고칠 때 또 열린다', () => {
  const 글 = /<script>([\s\S]*?)<\/script>/.exec(NV.쪽('ㄱ', 'ㄴ'))[1];
  assert.match(글, /replaceState/, '닫아도 주소에 자리표가 남는다');
});

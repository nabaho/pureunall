...
test('★★★ 문단이 «문서에 적힌 차례 그대로» 줄이 된다', () => {
  ...
  assert.ok(ls.indexOf('○ 성 명 : 홍길동') > 0);
  assert.ok(ls.indexOf('2026년 09월 18일') > ls.indexOf('○ 성 명 : 홍길동'),
...
test('★★★ 표는 «칸 | 칸 | 칸» 으로 나온다 — 실측 줄 그대로', () => {
  ...
  assert.ok(ls.includes('2 | ㈜카타에이씨 | 8.18.(화)'));
});
...
test('★★★ 주민등록번호를 «통째로» 가린다 — 생년 네 자리도 새면 안 된다', () => {
  ...
  assert.ok(t.indexOf('800101') < 0, '⚠ 앞 6자리(생년월일)가 남아 있습니다');
  assert.ok(t.indexOf('1234567') < 0, '⚠ 뒷자리가 남아 있습니다');
  assert.ok(t.indexOf('○○○○○○-○○○○○○○') > 0, '가렸다는 것이 보여야 합니다');
});

test('★★ 가리면서 «앞 글자»를 지우지 않는다', () => {
  assert.equal(T.maskRrn('○ 주민등록번호 : 800101-1234567'), '○ 주민등록번호 : ○○○○○○-○○○○○○○');
  assert.equal(T.maskRrn('800101-1234567 님'), '○○○○○○-○○○○○○○ 님', '맨 앞에 있어도 가린다');
  assert.equal(T.maskRrn('가800101-1234567'), '가○○○○○○-○○○○○○○');
});

test('★★ 나란히 둘이 있어도 둘 다 가린다', () => {
  assert.equal(T.maskRrn('800101-1234567, 801231-2345678'), '○○○○○○-○○○○○○○, ○○○○○○-○○○○○○○');
});
...
test('★★ 주민번호가 «아닌» 숫자는 건드리지 않는다 — 계좌·전화·사업자번호', () => {
  ...
  assert.equal(T.maskRrn('8001011234567'), '8001011234567', '붙여 쓴 13자리는 그대로');
  assert.equal(T.maskRrn('8001011-12345670'), '8001011-12345670', '자릿수가 다르면 아니다');
  /* ⚠ 뒤에 숫자가 «더» 이어지면 주민번호가 아니다 — 가리면 엉뚱한 번호가 토막 난 채 남는다
     (「800101-12345677」이 「○○○○○○-○○○○○○○7」이 된다). */
  assert.equal(T.maskRrn('800101-12345677'), '800101-12345677', '뒤가 8자리면 아니다');
});

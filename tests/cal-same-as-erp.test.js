/* ══ 【4걸음 알림 — 고친다】 ════════════════════════════════════
   이 파일은 푸른 캘린더를 «이알피와 맞대» 본다. 4걸음에서 맞댈 상대가 사라진다.
   그때는 규칙을 «홀로 서게» 다시 적어야 한다 — 예: 「출장은 남색, 병가는 빨강」을
   이알피에서 떠 오지 말고 여기에 값으로 적고, 왜 그 값인지를 함께 적는다.
   ⚠ 그냥 지우면 칩 글자·색 규칙을 아무도 안 지킨다.
...
test('★ 한 칸에 보이는 개수 — 둘 다 접되, 푸른 캘린더는 «재서» 정한다', () => {
  /* 이 파일의 나머지는 모두 「이알피와 같다」를 본다. 여기도 «접는다»는 같다.
     다른 것은 몇 개에서 접느냐다.
       이알피      : 넷 — 소스에 못 박아 둔 수(MAX_SHOW).

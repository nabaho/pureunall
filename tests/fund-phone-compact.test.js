...
test('★③ 접고도 «기업을 알아볼 칸»은 남는다 — 상호·업종·상시근로자', () => {
  ...
  for (const [자국, 무엇] of [
    /* 2026-09-14: 상호 칸 «안»에 대표사업장 별(★/☆)이 먼저 붙는다 — 칸 자체는 그대로다.
       그래서 여는 <td> 와 이름 사이에 다른 것이 끼어도 «같은 칸»이면 통과해야 한다
       (다음 <td> 를 넘어가면 옆 칸을 제 것으로 읽으므로 거기서 끊는다). */
    [/<td([^>]*)>(?:(?!<td)[\s\S])*?esc\(s\.name/, '상호'],

...
test('★★ 요약과 전문이 «같은 여백»을 쓴다', () => {
  ...
  const 재기 = (h) => {
    const m = /padding:30px (\d+)px 0 \1px;/.exec(h);   /* 꼬리 — 두 쪽에 다 있다 */
    assert.ok(m, '여백을 못 읽었다');
    return m[1];
  };
...
/* 살구 판이 몇 칸으로 놓였나.
   ⚠ <td> 를 세면 «카드 안»의 칸(표지·글)까지 세어 늘 3 이 나온다 — 처음에 그렇게
     짜서 아무것도 못 잡았다. 바깥 칸만 가리키는 것은 «폭 몫»(50% / 33%)이다. */
function 자료칸수(h) {
  const i = h.indexOf('background-color:#fbf4ea');
  assert.ok(i > 0, '자료 칸(살구 판)을 못 찾았다');
  const 판 = h.slice(i);
  if (판.indexOf('width="33%"') >= 0) return 3;
...
test('★★★ 넓게 주면 «넓게» 짓고 자료 카드를 셋씩 놓는다', () => {
  ...
  assert.equal(자료칸수(h), 3,
    '넓은데 두 칸이다 — 카드 높이는 표지가 정해서 오른쪽만 비고 줄 수는 그대로다');
});
...
test('★★ 웹 쪽 껍데기가 폭을 «지어진 전문에서» 읽는다', () => {
  ...
  assert.ok(쪽.indexOf('#wrap{width:' + Tpl.전문넓이 + 'px') >= 0, '껍데기 폭이 편지와 다르다');
});

...
function 자료칸수(h) {
  ...
  const 판 = h.slice(i);
  /* ⚠ 넓은 메일(2026-09-26~)은 «표로 셋씩»이다 — 33% 를 먼저 본다.
       50% 를 먼저 보면 기사 두단표의 칸에 걸려 늘 2 가 나온다. */
...
/* ★★★ 2026-09-26 에 «뒤집혔다» — 메일도 넓다
   ═══════════════════════════════════════════════════════════════════════════
   대표 지시 「몇번째 요청했는데 캡쳐3의 화면과 같이 좌우로 넓게 정리해서 보내는 방식으로」.
   2026-09-17 에 「좌우로 넓게」를 듣고 «웹만» 넓혔던 것이 이 검사에 굳어 있었다
   (「메일은 700 이라야 한다」). 실측해 보니 다음메일 읽는 칸은 1,600px 남짓인데
   편지만 700 이라 오른쪽이 비어 있었다.
   ⚠ 되돌리지 말 것 — 되돌리면 대표께서 네 번째로 같은 말씀을 하시게 된다.
   ★ 다만 «아웃룩이 모르는 것»은 여전히 메일에 못 쓴다 — 아래 검사들이 그것을 지킨다. */
test('★★★ 안 주면 «넓게» 짓는다 — 메일도 980 이다', () => {
  const h = 전문편지();
  assert.equal(폭읽기(h), Tpl.전문넓이, '메일이 아직 좁다 — 오른쪽이 통째로 빈다');
  assert.equal(자료칸수(h), 3, '넓은데 자료가 세 칸이 아니다');
});

test('★★★ 넓은 «메일»의 자료는 «표»로 셋씩이다 — 아웃룩은 칸틀을 모른다', () => {
  const h = 전문편지();
  assert.ok(h.indexOf('display:grid') < 0, '⚠ 메일에 칸틀(grid)이 샜다 — 아웃룩에서 무너진다');
  assert.ok(h.indexOf('width="33%"') >= 0, '표로 셋씩이 아니다');
});

test('★★★ «웹»으로 주면 칸틀로 셋씩 놓는다 — 단추 바닥이 맞는다', () => {
  const h = 전문편지({ 넓이: Tpl.전문넓이, 웹: true });
  assert.ok(Tpl.전문넓이 >= 900, '전문 폭이 안 넓다');
  assert.equal(폭읽기(h), Tpl.전문넓이, '준 폭으로 안 지었다');
  assert.equal(자료칸수(h), 3, '넓은데 세 칸이 아니다 — 오른쪽이 통째로 빈다');
});

test('★★ 좁게 지으면 예전처럼 둘씩 — 지난 회차를 다시 그릴 때', () => {
  const h = 전문편지({ 넓이: 700 });
  assert.equal(폭읽기(h), 700, '준 폭을 안 지킨다');
...
test('★★★ 넓은 쪽 자료 카드는 «줄과 열이 맞는다» (대표 지시 2026-09-18)', () => {
  ...
  const 판 = 자료판(전문편지({ 넓이: Tpl.전문넓이, 웹: true }));
...
test('★★ 마지막 줄이 덜 차도 «칸»은 만든다 — 안 만들면 가로줄이 도중에 끊긴다', () => {
  ...
  [4, 5, 7].forEach((몇) => {
    ...
    const 판 = 자료판(Tpl.편지짓기(d, { 회사이름: '푸른노무법인' },
      { 요약: false, 미리보기: true, 넓이: Tpl.전문넓이, 웹: true }).서식);
    ...
    /* ★ 넓은 «메일»(표)도 같아야 한다 — 안 그러면 마지막 줄의 가로줄이 도중에 끊긴다 */
    const 표판 = 자료판(Tpl.편지짓기(d, { 회사이름: '푸른노무법인' },
      { 요약: false, 미리보기: true }).서식);
    const 표칸수 = (표판.match(/width="33%"/g) || []).length;
    assert.equal(표칸수, Math.ceil(몇 / 3) * 3,
      '메일: 자료 ' + 몇 + '장인데 칸이 ' + 표칸수 + '개다 — 마지막 줄의 빈 칸을 안 만들었다');
  });
});

test('★★★ «웹»으로 한 번 지은 뒤에도 메일에 칸틀이 안 샌다', () => {
  /* 값을 모듈에 담아 두면 한 번 웹으로 지은 뒤 메일까지 웹인 줄 알고 그려진다 —
     그 편지는 아웃룩에서 카드가 통째로 무너진다.
     ⚠ 2026-09-26 까지 이 검사는 «폭»이 새는지 봤다. 이제 폭은 둘 다 980 이라
       샐 것이 없다 — 새면 안 되는 것은 «웹인가»다. */
  전문편지({ 넓이: Tpl.전문넓이, 웹: true });
  const h = 전문편지();
  assert.ok(h.indexOf('display:grid') < 0, '앞서 웹으로 지은 것이 메일에 새 나갔다');
  assert.ok(h.indexOf('column-count') < 0, '앞서 웹으로 지은 단이 메일에 새 나갔다');
});

test('★★ 웹 쪽 껍데기가 폭을 «지어진 전문에서» 읽는다', () => {
  /* 숫자를 두 곳에 적으면 한쪽만 바뀌어 쪽이 잘리거나 가운데로 쏠린다.
     ⚠ 옛 회차는 700 으로 담겨 있다 — 그때 담긴 대로 보여야 맞다. */
  assert.equal(NV.전문폭(전문편지({ 넓이: Tpl.전문넓이, 웹: true })), Tpl.전문넓이, '넓은 전문을 못 읽었다');
  /* ⚠ 옛 회차는 700 으로 담겨 있다 — 그때 담긴 대로 읽어야 맞다(지금 것은 980) */
  assert.equal(NV.전문폭(전문편지({ 넓이: 700 })), 700, '옛 폭(700) 전문을 못 읽었다');
  ...
  assert.equal(NV.전문폭('<table width="9999" style="width:9999px;background-color:#ffffff">'), 700,
    '터무니없는 폭을 그대로 믿는다');
  const 쪽 = NV.쪽('제목', 전문편지({ 넓이: Tpl.전문넓이, 웹: true }));
...
function 두꼴(안내용, 우리글) {
  ...
  const 짓 = (옵션) => Tpl.편지짓기(d(), { 회사이름: '푸른노무법인' },
    Object.assign({ 요약: false, 미리보기: true }, 옵션 || {})).서식;
  /* ⚠ 2026-09-26 부터 둘을 가르는 것은 «폭»이 아니라 «웹인가»다 — 폭은 둘 다 980 이다 */
  return { 메일: 짓(), 넓은: 짓({ 넓이: Tpl.전문넓이, 웹: true }) };
}

test('★★★ 넓은 쪽은 «단»으로 흘린다 — 메일에는 절대 안 샌다', () => {
  /* ⚠⚠ 아웃룩은 column-count 를 모른다. 하나라도 메일에 새면 그 편지는 한 줄로
       주르륵 쏟아진다 — 그래서 «없는지»를 본다. */
  /* ⚠ 기사는 «둘 이상» 담는다 — 하나뿐이면 두 칸으로 가를 것이 없어(두단표가 그대로
       돌려준다) 「안 가른다」로 걸린다. 실제 회차는 늘 여러 건이다. */
  const { 메일, 넓은 } = 두꼴({
    news: [1, 2, 3].map((i) => ({ 갈래: '기사', 한줄: '한 줄 ' + i,
      우리말: '우리가 쓴 글 ' + i, 언론사: '매일노동뉴스' })),
    policy: [1, 2, 3].map((i) => ({ 갈래: '자료', 제목: '자료 ' + i, 발행처: '고용노동부' })),
  }, '이번 주 한마디입니다.');
  /* ⚠⚠ 메일에는 «웹에서만 되는 것»이 하나도 없어야 한다 — 단·칸틀·늘임 모두.
       ★ 메일이 980 으로 넓어진 뒤(2026-09-26)에도 이 선은 그대로다. 넓어졌다고
         칸틀을 쓰면 아웃룩에서 카드가 통째로 무너진다 — 그 자리를 «표»가 맡는다. */
  ['column-count', 'break-inside', 'display:grid', 'display:flex'].forEach((것) => {
    assert.ok(메일.indexOf(것) < 0, '⚠ 메일에 ' + 것 + ' 이 샜다 — 아웃룩이 모른다');
  });
  /* 그 대신 메일은 «표»로 가른다 — 기사는 두 칸, 자료는 세 칸 */
  assert.ok(메일.indexOf('width="50%"') >= 0, '넓은 메일인데 기사를 두 칸으로 안 가른다');
  assert.ok(메일.indexOf('width="33%"') >= 0, '넓은 메일인데 자료를 세 칸으로 안 놓는다');

...
test('★★ ⑬ 기금 정보와 참여사업장이 같은 셈을 쓴다 — 따로 짜면 두 숫자가 갈린다', () => {
  ...
  /* ⚠ 2026-09-14: «거르기 전» 전체로 센다. 종전에는 거른 뒤(arr)를 세어, 딱지 하나만 눌러도
     「지자체 3」이 「지자체 1」로 바뀌었다 — 우리 기금의 지자체가 손 댈 때마다 달라졌다. */
  const st = grabFn('sitesTab');
  assert.match(st, /_siteGovs\(전체\)/, '참여사업장 머리에 지자체 수가 없습니다.');
  assert.ok(!/_siteGovs\(arr\)/.test(st),
    '★ 거른 뒤로 셉니다 — 딱지를 누를 때마다 지자체 수가 달라집니다.');
});

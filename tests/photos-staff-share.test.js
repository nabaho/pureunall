...
test('★ 콘솔과 «한 곳도» 다르지 않다', () => {
  ...
  /* ★ 2026-09-13(두 번째) — 서면함 셋이 «올라갔다». 경력관리가 기관 서식의 「노무법인」 칸을
       채우려고 /data/company_info 에 이름을 붙이면서 `rules-deploy.js --deploy` 를 돌렸는데,
       규칙은 «통째로» 갈아 끼우는 것이라 기다리던 셋도 함께 게시됐다.
     ★ 위에서 걱정하던 「콘솔이 main 보다 조여져 있다」는 «풀렸다» — 올리기 전 살아 있는
       콘솔과 견줘 **사라지는 규칙 0개**였다(#1225 가 main 에 들어온 뒤다). 안전장치가
       그것을 확인해 주었고, 올린 뒤 다시 읽어 콘솔이 올린 것과 같음도 확인했다.
     ⚠ 이 목록이 «비어 있는 것»이 정상이다. 비어 있지 않다는 것은 「콘솔에 아직 안 올라간
       규칙이 있다」는 뜻이고, 이 자리 말고는 그것을 알려 주는 곳이 없다. */
  const PENDING = [];

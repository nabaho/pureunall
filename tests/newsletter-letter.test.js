...
test('편지의 뼈대가 그대로 있다', () => {
  ...
  /* ⚠ 2026-09-23 부터 차림표는 «편지에 실제로 있는» 꼭지만 부른다 — 아래
       「빈 꼭지는 아예 안 그린다」 검사에 그 까닭을 적어 두었다.
     그래서 견본에 «담긴» 꼭지만 본다. */
  C.꼭지들.forEach((g) => {
    if (!((회차자료().안 || {})[g.키] || []).length && g.갈래 !== '우리글') return;
...
test('빈 꼭지는 «아예 안 그린다» — 제목만 덩그러니 남으면 흉하다', () => {
  ...
  /* ★★★ 2026-09-23 에 «뒤집었다» — 빈 꼭지는 차림표에서도 사라진다.
     ═══════════════════════════════════════════════════════════════════════
     예전에는 「차림표에는 남는다(늘 넷이다)」였다. 그런데 차림표는 웹 전문 보기에서
     «누르면 그 꼭지로 내려앉는 목차»다(functions/news-view.js 차림표스크립트).
     그 스크립트는 차림표 칸과 자리표(id="g-…")를 «번호로» 짝짓는데, 빈 꼭지는
     자리표가 안 생기므로 한 칸씩 밀렸다 — 실측(2026-09-23): 정책이 빈 회차에서
     「고용·노동정책」을 누르면 «판례»로 내려갔다. 고장인데 조용히 틀린다.
     ★ 게다가 편지에 없는 꼭지를 부르는 차림표는 그 자체로 거짓말이다.
     ⚠ 되돌리지 말 것 — 되돌리려면 news-view.js 를 «이름으로 짝짓게» 먼저 고쳐야 한다. */
  assert.ok(h.indexOf('판례·재결례') < 0, '빈 꼭지는 차림표에서도 사라져야 한다');
});

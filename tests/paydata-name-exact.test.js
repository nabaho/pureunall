...
/* ══════ 이름이 «그대로» 같은 곳이 먼저다 (2026-09-03) ══════
   ...
   ■ 다만 «이름으로 짐작하는» 자리가 하나 남아 있고, 거기서 진짜 결함을 찾았다
     ...
     그런데 긴 이름부터 훑으며 「담고 있나」만 보아서 —
       파일 「두레」  ·  목록 [두레, 두레가축약품]  →  «두레가축약품» 을 골랐다.
     목록에 「두레」가 버젓이 있는데 다른 회사를 고른 것이다.
     PR #837 이 「「두레」는 「두레가축약품」과 다른 곳」이라고 못 박은 그 쌍이다.

   ★ 여기서 못 박는 것
     ① 이름이 그대로 같은 곳이 «먼저»다 — 긴 이름 우선보다 앞선다
     ② 앞가지·괄호·빈칸을 뗀 뒤 견주는 것은 그대로다
     ③ 「지점」처럼 뒤에 붙는 것은 여전히 그 회사로 본다 (다온원 아산점 → 다온원)
     ④ 못 알아본 것은 «빈칸»으로 남긴다
     ⑤ ⚠ 「두레전자」가 「두레」로 걸리는 것은 «못 막는다» — 여기 적어 둔다
...
test('★ 이름이 «그대로» 같은 곳이 먼저다 — 긴 이름이 짧은 이름을 가로채지 않는다', () => {
  assert.equal(hit('두레'), '두레',
    '★ 목록에 「두레」가 있는데 「두레가축약품」을 골랐다 — 다른 회사 서랍에 들어간다');
});

test('★ 앞가지·괄호·빈칸을 뗀 뒤에도 «그대로 같음»으로 본다', () => {
  const list = [co('a', '두레'), co('b', '두레가축약품')];
  assert.equal(hit('주식회사 두레', list), '두레');
  assert.equal(hit('(주)두레', list), '두레');
  assert.equal(hit('두 레', list), '두레');
});

test('그대로 같은 것이 없으면 «담고 있는» 곳으로 물러난다', () => {
  assert.equal(hit('두레가축약품 8월.xlsx'), '두레가축약품');
  assert.equal(hit('두레 8월.xlsx'), '두레', '파일 이름이 「두레」로 시작하면 두레이다');
  assert.equal(hit('두레.xlsx'), '두레');
});

/* ── ③ 지점은 그 회사다 (이미 있던 약속) ────────────────────── */

test('★ 「지점」처럼 뒤에 붙는 것은 그 회사로 본다 — 급여관리 설정카드가 그렇게 적는다', () => {
  assert.equal(hit('다온원 아산점 8월'), '다온원');
  assert.equal(hit('다온원 아산점_25년 07월_급여대장.xlsx'), '다온원');
});

test('더 긴 이름이 맞으면 그것을 고른다 — 「다온원」이 「다온원산업」을 가로채지 않는다', () => {
  assert.equal(hit('다온원산업 8월'), '다온원산업');
});
...
test('★ 못 알아본 것은 «빈칸»으로 남긴다 — 아무거나 골라 넣으면 아무도 모른다', () => {
  ...
  assert.equal(load().matchCompanyName('다온원', null), null, '목록이 없어도 터지지 않는다');
  assert.equal(load().matchCompanyName('다온원', []), null);
});

test('★ 이름이 빈 업체가 «못 알아본» 파일을 가로채지 않는다', () => {
  /* ⚠ 빈 이름은 «무엇에나» 걸린다 — indexOf('') 는 늘 0 이다.
     ⚠ 「두레 8월」로 견주면 이 자리에 못 닿는다: 긴 이름부터 보므로 「두레」가 먼저
       맞아 빈 이름까지 가지 않는다. 아무도 안 맞는 파일이어야 그 자리에 닿는다
       (2026-09-03 고장넣기에서 이 검사가 헛돈 것을 잡았다). */
  const list = [co('x', ''), co('a', '두레')];
  assert.equal(hit('미르텍 8월', list), null,
    '★ 이름 없는 업체가 못 알아본 파일을 다 받아 간다 — 그 서랍이 쓰레기통이 된다');
  assert.equal(hit('두레 8월', list), '두레', '멀쩡한 것은 그대로 맞아야 한다');
});

/* ── ⑤ 못 막는 자리를 «적어 둔다» ───────────────────────────── */

test('⚠ 「두레전자」가 「두레」로 걸리는 것은 못 막는다 — 알고 두는 것이다', () => {
  /* 「다온원 아산점」이 다온원의 지점이라, «이름 뒤에 글자가 붙으면 다른 곳»으로
     ...
     ★ 이 검사는 「고쳐졌다」고 착각하지 않으려고 둔다. 언젠가 가릴 길이 생기면
       이 검사를 바꾸는 것으로 시작한다. */
  assert.equal(hit('두레전자 8월.xlsx'), '두레',

...
test('⑦★★ 백업에서 빼는 것은 «화면 상태»뿐이다 — 자료는 하나도 안 뺀다', () => {
  /* 대표 지시 2026-09-20 「다담아라」 — 두 번 말씀하셨다.

     ①② 는 «서버로 공유되는» 자료만 지킨다. 그런데 이 PC 에만 있는 자료도 있다
     (통장 작업 상태·업체 되돌리기 기록). 그것들이 빠져도 ①② 는 조용하다.

     그래서 다른 잣대를 쓴다 — **지우개가 아는 키**(_backupKnownKeys).
     이 목록은 옛 백업을 지울 때 «이 자리에 뭔가 실려 있을 수 있다» 고 코드가
     알고 있는 칸들이다. 거기 있는데 백업에서 빠진다면, 지우개가 «없는 것»을
     지우고 있다는 뜻이고 — 무엇보다 자료 한 칸이 조용히 안 담기고 있다는 뜻이다. */
  const fn = cutFn(SRC, 'function _backupKnownKeys(');
  const 아는것 = new Set(
    (fn.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/[^\n]*/g, '').match(/'([^']+)'/g) || [])
      .map(x => x.slice(1, -1))
  );
  /* 이 함수는 FB_ALL_SYNC_KEYS·SERVER_BACKUP_KEYS 도 합치므로 그 둘도 넣는다 */
  이름들('var FB_ALL_SYNC_KEYS = [').forEach(k => 아는것.add(k));
  이름들('var SERVER_BACKUP_KEYS = [').forEach(k => 아는것.add(k));
  assert.ok(아는것.size > 40, '★ 지우개가 아는 목록을 제대로 못 읽었다(' + 아는것.size + '개)');

  const 뺀것 = 백업에서빼는것();
  const 자료인데뺀것 = 뺀것.filter(k => 아는것.has(k));
  assert.deepEqual(자료인데뺀것, [],
    '★★ 자료인데 백업에서 빠지는 키가 있다: ' + 자료인데뺀것.join(', ') + '\n' +
    '  덩치가 걱정이면 빼지 말고 쪼개 실어라(erpBackupBatches 가 이미 그렇게 한다).\n' +
    '  정말 빼야 한다면 지우개(_backupKnownKeys)에서도 함께 빼고, 왜인지 적어라 —\n' +
    '  한쪽만 고치면 다음 사람은 담기는 줄 알고 안심한다.');

  /* 남은 것이 «화면 상태»인지도 한 번 더 본다 — 자료 이름은 이런 꼴이 아니다 */
  assert.ok(뺀것.length > 0 && 뺀것.length < 25,
    '★ 빼는 목록이 ' + 뺀것.length + '개다 — 화면 상태치고 너무 많다. 자료가 섞여 들어갔는지 보라');
});

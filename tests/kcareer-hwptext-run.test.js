...
function 한글무대(opts) {
  opts = opts || {};
  const 파일 = opts.파일 || { 'Contents/section0.xml': 구역('참석확인서 권형하 회의수당 1,600,000') };
  const 기록 = { 열린이름: null };
  const ctx = {
...
test('★★★ 주민등록번호를 «보내기 전»에 가린다 — 회의비를 읽는 데 필요 없다', async () => {
  ...
  assert.ok(보낸글.indexOf('750107') < 0, '★★★ 주민번호가 AI 로 나갔습니다');
  assert.ok(보낸글.indexOf('1684916') < 0);
  assert.ok(보낸글.indexOf('1,600,000') > 0, '정작 읽어야 할 금액은 그대로 가야 합니다');
});

test('★★★ 주민번호를 «묻는» 화면에서는 가리지 않는다 — 그 화면은 그것을 읽어야 한다', async () => {
  const m = 판독무대('{"number":"주민번호앞7자리"}', '{"number":"750107-1"}');
  await m.돌리기();
  const 보낸글 = m.기록.보낸것.map(function (p) { return p.text; }).join('\n');
  assert.ok(보낸글.indexOf('750107-1684916') > 0, '⚠ 가리면 신분증 화면이 영영 못 읽습니다');
});

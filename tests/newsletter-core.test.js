...
/* ⚠ 개수를 못 박지 않는다 (2026-09-23). 「넷」으로 박아 두었더니 특별 판례 꼭지를
     더하는 «멀쩡한 일»에 이 검사가 깨졌다 — 지키려던 것은 개수가 아니라
     «우리가 쓰는 칸이 있는가»였다. 키가 겹치지 않는지는 새로 본다(겹치면 뒤엣것이 이긴다). */
test('우리 글 꼭지가 «하나» 있고, 꼭지 키가 겹치지 않는다', () => {
  assert.ok(C.꼭지들.length >= 4, '꼭지가 너무 적다');
  const 우리 = C.꼭지들.filter((g) => g.갈래 === '우리글');
  assert.equal(우리.length, 1, '우리가 쓰는 칸이 없으면 우리 뉴스레터가 아니다');
  const 키 = C.꼭지들.map((g) => g.키);
  assert.equal(new Set(키).size, 키.length, '꼭지 키가 겹치면 뒤엣것이 앞엣것을 덮는다');
});

/* ⚠ 2026-09-03 대표께서 «원본»(8월 5주차 받은 것)을 보내 주셨다. 그 전에는 첫 꼭지에만
     Best 를 붙이고 나머지는 맨 제목이었는데, 원본은 넷 다 영문 딱지를 달고 있었다. */

test('★ 꼭지 «모두»에 영문 딱지가 붙는다 — 원본이 그렇다', () => {
  C.꼭지들.forEach((g) => {
    assert.ok(String(g.딱지 || '').trim(), '«' + g.이름 + '» 에 영문 딱지가 없다');
  });
  /* 검사고정-허용: 원본(8월 5주차 받은 것)에 실제로 붙어 있던 네 낱말이다.
     값이 곧 규칙이다 — 이것이 다르면 「완전히 일치하는 디자인」이 아니다.
     ⚠ 새 꼭지가 늘어도 이 넷은 «이 차례로» 남아 있어야 한다. 그래서 목록을
       통째로 견주지 않고 «넷이 이 차례로 들어 있는가»를 본다(2026-09-23). */
  const 있는것 = C.꼭지들.map((g) => g.딱지)
    .filter((d) => ['Best', 'ISSUE', 'Attention', 'Trend'].indexOf(d) >= 0);
  assert.deepEqual(있는것, ['Best', 'ISSUE', 'Attention', 'Trend']);
});
...
test('막은 주소 목록(공용 수신거부)도 함께 본다', () => {
  /* 실시간DB 열쇠에는 점을 못 쓴다 — 담는 곳마다 바꿔 넣는 글자가 다르다 */
  const r = C.명단다듬기([{ email: 'x@y.com' }], { 'x@y_com': 1 });
  assert.equal(r.셈.수신거부, 1, '밑줄 꼴로 막은 주소가 뉴스레터로 새 나갔다');
});

/* ★ 2026-09-20 전수점검 — 여태 «아무도 안 담는 꼴»만 지키고 있었다.
     기업정보함(pu-cards.html emailKey)은 점을 «쉼표»로 바꿔 담는다(a@b.com → a@b,com).
     위 검사는 밑줄 꼴만 봤으므로, 대표가 「수신거부」를 눌러도 그대로 나갔을 것이다.
     명단이 아직 비어 있어 실제 피해는 없었다 — 비어 있을 때 막는다. */
test('기업정보함이 담는 «쉼표 꼴»로 막은 주소도 걸러진다', () => {
  const r = C.명단다듬기([{ email: 'x@y.com' }], { 'x@y,com': 1 });
  assert.equal(r.셈.수신거부, 1,
    '기업정보함에서 「수신거부」를 누른 주소가 뉴스레터로 그대로 나간다');
  assert.equal(r.셈.보낼곳, 0);
});
...
test('★ 공인노무사회 양식 그대로 — 번호·대상자·메일주소·비고', () => {
  const r = C.노무사회양식([
    { company: '가나시시설관리공단', name: '홍길동', email: 'a@x.kr', title: '과장' },
    { company: '파하디티(주)', name: '김담당', email: 'b@x.kr', title: '' }
  ]);
  assert.deepEqual(r.머리, ['번호', '대상자', '메일주소', '비고']);
  assert.deepEqual(r.줄들[0], ['1', '가나시시설관리공단', 'a@x.kr', '홍길동 과장']);

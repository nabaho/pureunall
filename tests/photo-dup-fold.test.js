...
/* 똑같은 서류는 «한 줄»로 접는다 (대표 지시 2026-09-12 「완전 동일서류는 중복 정리해라」)
   ...
   ■ 무엇이 있었나 — 운영 자료 그대로
     「사진첩에서 계약서 찾기」에 나라어린이집 자문계약서가 **세 줄** 있었다.
     세어 보니 서로 다른 셋이 아니었다:
...
/* ── 운영 자료 그대로의 세 장 (나라어린이집, 2026-09-12 읽음) ──
   ⚠ 사람 이름·주소는 뺐다. 접기가 보는 칸은 아래가 전부다. */
const 계약 = { company:'나라어린이집', docName:'자문계약서', signDate:'2026-08-28',
...
test('⑦ ★ 띄어쓰기 차이는 «같은 서류»로 본다 — 판독이 늘 똑같이 떼어 쓰지 않는다', function () {
  const 띈것 = Object.assign({}, 셋[0], { id:'S', used:null, doc:null,
    fields:Object.assign({}, 계약, { company:'나라 어린이집', fee:'월200,000원' }) });

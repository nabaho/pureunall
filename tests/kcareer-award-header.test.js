...
test('★ 어학(성적·등급)으로는 «채우지 않는다» — 담는 자료가 없다', () => {
  /* ⚠ 이 검사는 처음에 「자격·상벌도 채우지 않는다」였다. 그때는 그것이 맞는 규칙이었다 —
     서류에 무엇이 나갈지는 사람이 정할 일이라 여쭤 본 뒤 정하기로 했었다.
     대표 결정 2026-09-13 「채워라」로 자격·상벌은 채운다. 어학은 그대로 둔다:
     어학 점수를 담는 자료가 아예 없어서, 채우게 하면 빈 값이 자리만 차지한다. */
  assert.equal(X.LIST_FILL_KEYS.indexOf('grade'), -1,
    '어학 점수를 담는 자료가 없는데 채우게 되었습니다 — 빈 값이 자리만 차지합니다');
  ['certName', 'gotAt', 'awardWhat', 'awardOrg'].forEach((k) => {
    assert.ok(X.LIST_FILL_KEYS.indexOf(k) >= 0,
      k + ' 로 채우지 않습니다 — 대표 결정은 「채워라」입니다');
  });
});

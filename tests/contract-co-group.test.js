 ...
 *
 * 대표가 겪은 일: 「청화」를 치면 가나공사 / 합자회사 가나공사 / (자)가나공사가
...
test('★ (자) 도 법인 표기로 본다 — 합자회사의 줄임말이다', () => {
  assert.strictEqual(G.pcNormCo('(자)가나공사'), '가나공사');
  assert.strictEqual(G.pcNormCo('합자회사 가나공사'), '가나공사');
  assert.strictEqual(G.pcNormCo('자애병원'), '자애병원', '이름 속 「자」는 지우면 안 된다');
});

test('★ 사람 줄은 «사람 이름»이 아니라 그 사람의 회사로 묶인다', () => {
  const gs = group([co('가나공사', '123-81-20196'), person('김상호', '가나공사')]);
  assert.strictEqual(gs.length, 1, '사람 이름으로 따로 떨어지면 안 된다');
  assert.strictEqual(gs[0].name, '가나공사');
});
...
test('묶은 «다른 이름»을 모두 들고 있다 (화면에 적어 보이려고)', () => {
  ...
  assert.strictEqual(others.join(','), '(자)가나공사,합자회사 가나공사');
});

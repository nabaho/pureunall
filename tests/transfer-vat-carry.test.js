...
test('⑤ ★★ 바로잡기는 «금액을 건드리지 않는다» — 표시 한 칸만 바꾼다', function () {
  ...
  const 쓰는곳 = 고침.slice(고침.indexOf('async function fix'), 고침.indexOf('async function fix') + 1600);
...
test('⑧ ★★ 고쳤다고 «말하기 전에 다시 읽어» 본다 — 돌아온 값을 믿지 않는다', function () {
  /* ⚠★ 2026-09-14 대표 보고로 드러났다. 첫 판은 dbPatch 가 돌려준 참/거짓만 세어
       「5칸 고침 · 4칸은 대상을 못 찾았습니다」라고 적었다. 그런데 서버를 직접 읽어 보니
  ...
  assert.match(고침, /다시\[x\.flag\]/,
    '★★ 표시가 «실제로 붙었는지»를 안 봅니다');
  /* 「대상을 못 찾았습니다」로 단정하지 않는다 — 모르면서 단정하면 없는 기록을 찾아 헤맨다 */
  assert.ok(!/대상을 못 찾았습니다/.test(고침),
    '★★ 무엇이 문제인지 모르면서 「대상을 못 찾았다」고 단정합니다 — 대표가 없는 기록을 찾게 됩니다');
  assert.match(고침, /서버에서 다시 받기/,
    '★ 안 붙었을 때 «해 볼 일»을 안 알려 줍니다');
});

...
test('②-2★★ 소스 호스트 이름은 «별표»다 — DSM 이 IP 를 거절한다 (대표 화면 2026-09-18)', () => {
  /* 대표님이 192.168.0.21 을 넣으시자 「잘못된 도메인 이름」 빨간 딱지가 떴다.
     역방향 프록시의 소스 호스트 이름은 «도메인»만 받는다. 우리 나스에는 도메인이 없으므로
     별표(어느 이름으로 와도)뿐이다 — IP 를 시키면 그 자리에서 막히고 사람은 더 못 간다. */
  const p = plan({ host: '192.168.0.21', httpsPort: '5001' });
  const 칸 = p.칸.find(k => k.이름 === '소스 · 호스트 이름');
  assert.equal(칸.값, '*',
    '★★ IP 를 시키면 DSM 이 「잘못된 도메인 이름」으로 거절합니다 — 시킨 대로 했는데 안 되는 자리입니다');
  assert.match(칸.곁 || '', /IP/, '★ 왜 별표인지 같은 줄에 없으면 사람은 IP 를 다시 넣어 본다');
  assert.match(NAS, /곁 && h\('span'/, '★ 곁말을 화면이 안 그리면 적어 둔 뜻이 없다');
});

test('②-3 대상 프로토콜은 HTTPS 이고, 기본값이 HTTP 라는 것을 말한다', () => {
  const p = plan({ host: 'h', httpsPort: '5001' });
  const 칸 = p.칸.find(k => k.이름 === '대상 · 프로토콜');
  assert.equal(칸.값, 'HTTPS');
  assert.match(칸.곁 || '', /HTTP/,
    '★ 창을 열면 HTTP 로 놓여 있다 — 말 안 하면 그대로 저장하고 또 안 된다(대표 화면에서 실제로 그랬다)');
});
...
test('⑥★★ 원인이 CORS 일 때만 편다 — 그런데 «글월»이 아니라 «갈래»로 가른다', () => {
  /* 2026-09-18 저녁 대표 화면: 원인이 CORS 로 짚혔는데 안내판이 안 떴다.
     옛 코드가 화면 글월에서 /CORS/ 글자를 찾았기 때문이다. 그 글월에는 그 글자가 없다 —
     사람 말로 「DSM 이 이 화면의 주소를 안 받아들입니다」라고 적었으니까.
     글월은 사람이 읽으라고 고치는 것이고, 고치는 순간 글자로 하는 되짚기는 조용히 죽는다. */
  assert.match(NAS, /진단\.kind === 'cors'/,
    '★★ 갈래로 갈라야 한다 — 글월에서 낱말을 찾으면 글월을 고칠 때마다 조용히 죽는다');
  assert.ok(!/\/CORS\/\.test\(진단/.test(NAS),
    '★ 글자로 되짚는 길이 남아 있으면 둘 중 하나만 고쳐진다');
  const t = stripJs(cutFn(ERP, 'function doTest('));
  assert.match(t, /kind: r && r\.kind/, '★ 갈래를 안 남기면 화면이 가를 수가 없다');
});

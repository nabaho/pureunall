/* ══════ 🔗 같은 곳인 두 줄을 «사람이 짚어» 합친다 (대표 지시 2026-09-12) ═══════════
   ...
   ★ 못 박는 것
     ①⚠⚠ **짐작으로 안 합친다.** 이름이 겹쳐도 다른 곳인 쌍이 실제로 있다
        (「두레」/「두레전자」·「다온원」/「다온원 아산점」). 사람이 짚어 준 것만 따른다.
...
test('★★★ 이미 있던 「상호↔번호 다리」는 그대로다', () => {
  const 한줄 = build([ biz('b1','1238620021','주식회사 가나'), card('c1','가나') ]);
...
test('★★ 번호가 다른 등록증 둘은 «따로» 선다 — 다른 사업자다', () => {
  const r = build([ biz('b1','1238120012','가나'), biz('b2','4118612345','가나') ]);
...
test('★★★ 이름이 다르게 적힌 두 줄은 «저절로» 안 합쳐진다 — 짐작하지 않는다', () => {
  ...
  assert.equal(r.length, 2,
    '★★★ 이름이 겹친다고 합치면 「두레」과 「두레전자」가 한 줄이 된다');
});

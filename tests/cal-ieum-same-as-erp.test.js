...
test('외부인은 externalId 로 가른다 — 두 앱이 같은 눈으로 본다', () => {
  ...
  /* ⚠ 2026-09-20 — 이알피 쪽과 맞대던 줄을 뺐다. 그 화면을 걷어냈다.
     차례(외부 → 내부)가 규칙이다 — 외부 인원은 사번이 없어 반대로 보면 항상 빈값이 된다. */
...
test('변호사는 직책에 「변호사」가 들어가는지로 가른다', () => {
  ...
  /* ⚠ 2026-09-20 — 이알피 쪽과 맞대던 줄을 뺐다.
     검사고정-허용 — 직책에 「변호사」가 들어가는지로 가르는 것이 규칙이다.
     변호사는 직책 이름이 여러 가지라(「고문변호사」·「담당변호사」) 통째로 맞추면 빠진다. */
...
test('그만둔 사람은 기본으로 숨기되 «몇 명인지»는 늘 보인다', () => {
  ...
  /* ⚠ 글자(「그만둔」·「퇴사」…)는 못 박지 않는다 — 이알피 용어로 다듬어도 안 깨지게.
     지키는 것은 «숨긴 사람 수를 세고, 화면에 그 개수를 보여 주는가»다. */
  assert.match(캘린더, /filter\(function\(p\)\{\s*return p\.left;\s*\}\)\.length/,
    '그만둔 사람 수를 세지 않습니다');
  assert.match(캘린더, /명/, '몇 명인지 글자(명)로 안 보여 줍니다');
});

// ── 이제는 «여기서 다 한다» ───────────────────────────────────────────
/* ★ 2026-09-20 에 뜻이 뒤집혔다.
   예전 규칙: 이 화면은 «읽기»였다. 그래서 「못 한다」고 막지 말고 «저기서 한다»고
     이알피로 보내라 — 조용히 안 되는 것이 가장 나쁘기 때문이다.
   지금 규칙: 2걸음에서 넷을 다 옮기거나 지웠다(한도·강의는 지움, 링크 발급·발행·
     비고는 옮김). 그래서 보낼 일이 없다 — 되돌아가는 길이 «하나도» 없어야 한다.
   ⚠ 옛 규칙의 뜻(조용히 실패하지 않는다)은 그대로다. 못 하는 일이 다시 생기면
     그때는 다시 «갈 곳»을 알려 주어야 한다. */
test('이음 화면 일을 여기서 다 한다 — 이알피로 돌려보내는 길이 없다', () => {
  const n = (캘린더.match(/data-erp=\\?["']dash\/ieum/g) || []).length;

/* 일반 서식 — 아는 종류가 아니어도 모든 칸을 읽는다 (대표 지시 2026-08-13)
   ...
   실사례: 정부지원 신청서(카타엔지니어링)가 「서류로 보이지 않음」 —
...
test('★ 서식의 사업자번호는 있을 때만 검산한다 — 실제로 돌려 본다', async () => {
  ...
  /* 진짜 번호(체크섬 통과) — 화면 캡처의 123-81-20157 */
  const ok = await boot().afterRead({ kind: 'form', company: '가야', bizno: '1238120157',
    pairs: [{ k: '매출액', v: '32억' }] });
  assert.equal(ok.bizNoOk, true, '맞는 번호가 검산을 통과해야 합니다');
  assert.equal(ok.fields.bizno, '123-81-20157', '보기 좋은 꼴로 바꿔 담아야 합니다');
...
const FORM = { id: 'p1', meta: { read: { kind: 'form', fields: {
  docName: '정부지원 신청서', company: '㈜카타엔지니어링', bizno: '123-81-20157',
  ceo: '김대표', name: '박담당',
  pairs: [
    { k: '업체명', v: '㈜카타엔지니어링' },
    { k: '업종 / 주생산품', v: '제조업 / 2차전지장비' },
    { k: '사업자등록번호', v: '123-81-20157' },
    { k: '대표자', v: '김대표' },
    { k: '매출액(직전년도)', v: '32(억 원)' },
    { k: '담당자 성함', v: '박담당' },
    { k: '빈 칸', v: '  ' }
...
test('★ 복사가 화면 표와 같은 차례로 나간다 — 실제로 돌려 본다', () => {
  ...
  ctx.formCopy();
  assert.equal(copied,
    ['업체명\t㈜카타엔지니어링', '업종 / 주생산품\t제조업 / 2차전지장비',
     '사업자등록번호\t123-81-20157', '대표자\t김대표',
...
test('★ 서식의 칸 이름·값이 찾기에 걸린다', () => {
  assert.match(fnOf(app, 'hayOf'), /Array\.isArray\(f\.pairs\)/,
    '「카타엔지니어링」이나 「매출액」으로 치면 이 서식이 나와야 합니다');
});

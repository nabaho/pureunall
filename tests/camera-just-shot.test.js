...
/* 푸른카메라 — 「방금 찍은 것」을 «그림»으로 보여 준다 (대표 지시 2026-09-12)
   ...
   ■ ★ 촬영 코드는 «하나»다 (2026-08-08 대표 결정 「카메라는 하나로」)
   푸른카메라(pu-camera.html)는 문패일 뿐이고 촬영은 사진첩 것 하나를 쓴다.
   그래서 이 줄은 **어느 문으로 들어가도 똑같이** 나온다. 카메라를 두 벌로
   만들지 않는다 — 그때 같은 명함이 어느 카메라로 찍히느냐에 따라 다른 곳으로 갔다. */
...
test('★★★ 푸른카메라는 문패일 뿐 — 촬영 코드를 두 벌로 만들지 않는다', () => {
  const cam = fs.readFileSync(path.join(R, 'pu-camera.html'), 'utf8');
  assert.match(cam, /location\.replace\('pu-photos\.html\?/,
    '★★★ 푸른카메라가 제 촬영 코드를 갖기 시작하면, 2026-08-08 에 없앤 문제가 돌아옵니다 —\n' +
    '  같은 명함이 «어느 카메라로 찍었느냐»에 따라 다른 곳으로 갑니다.');
  assert.ok(!/getUserMedia/.test(cam), '★★★ 푸른카메라 안에 촬영 코드가 생겼습니다');
});

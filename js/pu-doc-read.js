...
(function (global) {
  ...
  /* ══ 국세청이 준 한 줄을 어떻게 읽는가 (대표 지시 2026-09-17) ═══════════════════
     ...
     ⚠ 체크섬으로는 못 막는다. 실제로 당했다 — `123-45-67891` 은 검산을 «통과한다»
       (앞 아홉 자리 가중합 169 → 검사자리 1, 적힌 것도 1). 그런데 국세청에는 없는
  ...
  var READ_VERSION = 18;  // …/ 11 = 글자 있는 PDF 는 글자로 판독(그림 왕복 없음) / 12 = 통장·계좌(bankbook) 갈래 추가 / 13 = 통장·근로자서류 다섯이 KINDS 에 빠져 other 로 굳던 것을 고침 + 급여명세서 이름 / 14 = 근로계약서(wcontract) 갈래 / 15 = 근로계약서의 임금(사람이 확인해야 산다) / 16 = … / 17 = 한 묶음 안의 «서로 다른 서류»를 쪽마다 갈라 읽는다(docs[]) / 18 = 그 docs 꼴을 «못 박았다» — 권하기만 해서는 안 먹혔다(실측)
  ...
  /* ⚠⚠★ 2026-09-12 둘째 판 — «답의 꼴»을 못 박는다.
       첫 판은 「서로 다른 서류면 갈라 주세요」라고 **권하기만** 했고, 안 먹혔다.
       위 PROMPT_ALL 은 첫 줄부터 「이 이미지가 어떤 서류인지」로 시작해 내내
       «한 장·한 벌»을 전제로 쓰여 있다 — 맨 끝에 붙인 한 문단이 그 전체를 못 이긴다.
     ★ 실측(1쪽 자문계약서 + 2쪽 자동출금 이용신청서, 같은 문서를 두 번):
         첫 번째 → 두 쪽 다 contract (은행·계좌가 통째로 사라짐)
         두 번째 → 두 쪽 다 cms      (계약기간·자문료가 통째로 사라짐)
       갈리지 않고 **한쪽이 늘 사라졌다.**
     ★ 그래서 «조건부»를 없앤다 — 여러 쪽이면 **언제나** docs 꼴이다.
       한 서류여도 docs 한 칸에 담는다. 고를 꼴이 하나뿐이면 흔들리지 않는다.
     ⚠ 「위에서 말한 꼴로 답하지 말라」를 **명시한다.** 안 적으면 더 길고 더 자주
       되풀이된 쪽(PROMPT_ALL)을 따른다. */
  var MULTI_NOTE =
    '\n\n═══════════════════════════════════════' +
    '\n⚠⚠ 지금부터가 **가장 중요한 규칙**입니다. 위 설명과 어긋나면 **이 규칙이 이깁니다.**' +
    '\n═══════════════════════════════════════' +
    '\n\n위 설명은 그림이 «한 장»일 때를 말한 것입니다.' +
    ' 지금은 한 묶음으로 스캔한 **여러 쪽**이며, 첫 그림이 1쪽이고 쪽 순서대로 놓여 있습니다.' +
    '\n\n■ 답의 꼴 — **반드시 이 꼴이어야 합니다.**' +
    '\n{"docs":[{"pages":[1],"kind":"…","fields":{…}},{"pages":[2],"kind":"…","fields":{…}}]}' +
    '\n\n⚠ 맨 바깥에 kind 를 두는 꼴(예: {"kind":"contract","company":"…"})로 답하지 «마세요».' +
    ' 여러 쪽일 때는 **틀린 답**입니다. 반드시 docs 배열로 감싸 주세요.' +
    '\n⚠ 서류가 하나뿐이어도 docs 배열입니다 — 칸이 하나인 배열로 답하세요.' +
    '\n\n■ docs 를 어떻게 나누나' +
    '\n· 여러 쪽이 **한 서류**이면(계약서처럼 조항이 이어지면) **한 칸**으로 합치고' +
    ' pages 에 그 쪽을 모두 적습니다. 항목이 여러 쪽에 흩어져 있으면 찾아서 채웁니다.' +
    '\n  예) 3쪽짜리 계약서 → {"docs":[{"pages":[1,2,3],"kind":"contract","fields":{…}}]}' +
    '\n· 한 묶음에 **서로 다른 서류**가 들어 있으면 **서류마다 한 칸**입니다.' +
    '\n  예) 1쪽 자문계약서 + 2쪽 자동출금 이용신청서' +
    ' → {"docs":[{"pages":[1],"kind":"contract","fields":{…}},{"pages":[2],"kind":"cms","fields":{…}}]}' +
    '\n\n■ 지킬 것' +
    '\n· kind 와 fields 는 위에서 말한 규칙 그대로, **서류마다 따로** 채웁니다.' +
    '\n· 한 서류의 fields 에 **다른 서류의 값을 섞지 마세요.**' +
    ' 계약서 칸에 은행·계좌를 넣거나, 자동이체 칸에 계약기간을 넣으면 안 됩니다.' +
    '\n· pages 는 1부터 센 쪽 번호입니다. 모든 쪽이 어느 한 서류에는 들어가야 합니다 —' +
    ' 빠뜨리지 마세요.' +
    '\n· JSON 외 텍스트 금지.';
  ...
  /* ══════ 한 줄 요약 (대표 지시 2026-08-29) ══════
     ...
     했다. 파일 이름이 「직현병국퇴.pdf」·「@@근로계약서(2026)-가온팜--.xls」 같아서
     이름만으로는 아무것도 알 수 없다.
  ...
  var TEXT_NOTE =
    ...
    ' 여러 쪽이면 「--- N쪽 ---」 로 나뉘어 있습니다.';

  /* 글자 길에서 «여러 쪽»일 때 덧붙이는 말 — 그림 길의 MULTI_NOTE 와 **같은 규칙**이다.
     ⚠ 두 벌로 두면 한쪽만 고쳐 놓고 다른 쪽은 옛 규칙으로 남는다(이 파일이 여러 번
       못 박아 둔 원칙이다). 그래서 쪽 세는 말만 갈아 끼우고 나머지는 그대로 쓴다. */
  var TEXT_MULTI_NOTE = MULTI_NOTE
    .split('첫 그림이 1쪽이고 쪽 순서대로 놓여 있습니다.')
    .join('「--- N쪽 ---」 로 쪽이 나뉘어 있습니다.')
    .split('위 설명은 그림이 «한 장»일 때를 말한 것입니다.')
    .join('위 설명은 서류가 «한 쪽»일 때를 말한 것입니다.');
  ...
  function readDocText(text, opts) {
    ...
    /* 여러 쪽이면 그림 길과 **같은 규칙**으로 서류를 가른다(2026-09-12).
       쪽 나눔표가 있으면 여러 쪽이다 — 없으면 한 쪽이라 예전 그대로 간다. */
    var 여러쪽 = /---\s*\d+쪽\s*---/.test(body);
    return runDocParts(
      [{ text: PROMPT_ALL + TEXT_NOTE + (여러쪽 ? TEXT_MULTI_NOTE : '') + '\n\n' + body }],
      'text', opts);
  }
  ...
  /* ── pairs 에만 담긴 값을 «이름 붙은 칸»으로 되메운다 (2026-08-26) ──
     ...
     대표 캡처의 실사례: 「4·4 제도 도입기업 선정 신청서」(파하테크)에서 화면에는
     사업자등록번호 123-81-20138 이 **또렷이 보이는데** 그 아래에는
  ...
  var PAIR_TO_KEY = {
    ...
    /* ── 신청서의 «담당자» (대표 지시 2026-09-18 「읽히게」) ────────────────
       기술보호·현장클리닉 신청서에는 담당자 이름·부서·직위·연락처가 한 표로 들어 있다.
       판독은 그것을 pairs 에 그대로 담아 왔는데(실측 2026-09-18: 8곳), 이름표가
       제각각이라(담당자명·담당자 성함·담당자 성명·담당부서·담당자 부서 …)
       **어느 칸에도 못 갔다** — 읽어 놓고 버리는 셈이었다.
       ⚠ 이름은 name 그대로 둔다 — 서식이 명함이 되는 길(formHasContact)이 이 칸을 본다.
       ⚠ 연락처를 «회사» 칸에서 갈라내는 일은 여기가 아니라 기업 상세로 보낼 때 한다
         (js/pu-doc-file.js 의 contactKeys) — 명함으로 갈 때는 그 사람 것이 맞기 때문이다. */
    담당자: 'name', 담당자명: 'name', 담당자성함: 'name', 담당자성명: 'name',
    담당자이름: 'name',
    담당부서: 'dept', 담당자부서: 'dept',
    담당자직위: 'title', 담당자직책: 'title',
    담당자이메일: 'email', 담당자메일: 'email',
    담당자휴대전화: 'mobile', 담당자휴대폰: 'mobile',
    담당자유선: 'tel', 담당자전화: 'tel', 담당자전화번호: 'tel', 담당자연락처: 'tel',
    직위: 'title', 직책: 'title',
  ...
  function bizregParse(text) {
    ...
    /* ⚠⚠ **사람 이름이 아닌 말**을 대표자로 담지 않는다 (대표 보고 2026-09-14).
       등록증 아래쪽에는 「발 급 사 유 : 대표자 변경」이 찍힌다. 위 첫 규칙은 그 줄에서도
       「대표자」 뒤의 두 글자를 집어 **ceo = 「변경」**으로 담았다 — 실제로 그렇게 들어가
       있었다(차카검수 등록증). 대표자 라벨이 흐려 안 읽힌 등록증에서는 이 줄이
       «유일하게 남은 대표자 글자»라 반드시 걸린다.
       ⚠ 막는 것은 «낱말»이지 자리가 아니다 — 사유 줄을 통째로 건너뛰면 사유 줄에
         진짜 이름이 적힌 경우(「대표자 홍길동 으로 변경」)를 놓친다. */
    var 사유말 = /^(변경|정정|신규|경정|이전|폐업|휴업|정정신고|재발급|말소|등록|사유|없음|해당)$/;
    var 좋은이름 = function (s) {
      var t = String(s || '').trim();
      return /^[가-힣]{2,5}$/.test(t) && !사유말.test(t);
    };
    for (i = 0; i < ceoRe.length && !out.ceo; i++) {
      m = compact.match(ceoRe[i]);
      if (m && m[1] && 좋은이름(m[1])) out.ceo = m[1].trim();
    }
    if (!out.ceo) {
      for (i = 0; i < lines.length - 1; i++) {
        if (/대\s*표\s*자/.test(lines[i]) && 좋은이름(lines[i + 1].replace(/\s/g, ''))) {
          out.ceo = lines[i + 1].replace(/\s/g, ''); break;
        }
        var sameLine = lines[i].match(/대\s*표\s*자\s+([가-힣]{2,5})/);
        if (sameLine && 좋은이름(sameLine[1])) { out.ceo = sameLine[1].trim(); break; }
      }
    }
    if (out.ceo) {
    ...
    /* ══ ⑥ 주소 ══ (2026-09-14 다시 씀 — 대표 보고 「ocr 한거 주소가 빠진적도 있다」)

       ■ 무엇이 빠졌나 — 실측
       차카검수 사업자등록증(2026-09-14, 무료 판독 길)에서 **소재지가 통째로 안 담겼다.**
       문서는 또렷했고 사람 눈에는 두 줄이나 보인다. 까닭이 둘이었다:

       ① **시·도 목록에 «정식 명칭»이 없었다.** 국세청 등록증은 늘 「충청남도」로
          찍히는데 목록에는 「충남」만 있었다 — 충청·전라·경상 도의 등록증은
          둘째 규칙에 아예 안 걸린다.
       ② **첫 규칙이 「본 점 소 재 지」 줄의 콜론에서 막혔다.** 담는 글자 목록에
          «콜론»이 없어서, 사업장 소재지 뒤에 본점 소재지가 한 줄 더 오는 «법인»
          등록증에서는 lookahead 까지 닿지 못하고 통째로 실패한다.
          (개인사업자 등록증에는 그 줄이 없어 여태 안 드러났다.)
       ③ 걸리더라도 «다음 칸 이름»이 꼬리에 붙었다 —
          「충남 … 103 본 점 소 재 지」 처럼. 그대로 업체관리로 들어간다.

       ■ 이제 하는 일 — **줄에서 먼저 찾는다.**
       OCR 이 주는 것은 줄이다. 라벨이 있는 줄을 잡아 라벨 뒤를 취하면 콜론 문제도,
       꼬리 문제도 생기지 않는다. 줄에서 못 찾을 때만 예전의 두 규칙으로 물러선다. */

    /* 시·도 — **정식 명칭을 먼저** 적는다(긴 것이 먼저 걸려야 「서울」이 「서울특별시」를 자르지 않는다). */
    var SIDO = '서울특별시|부산광역시|대구광역시|인천광역시|광주광역시|대전광역시|울산광역시' +
      '|세종특별자치시|경기도|강원특별자치도|강원도|충청북도|충청남도|전북특별자치도|전라북도|전라남도' +
      '|경상북도|경상남도|제주특별자치도' +
      '|서울|부산|대구|인천|광주|대전|울산|세종|경기|강원|충북|충남|전북|전남|경북|경남|제주';

    /* 꼬리에 딸려 온 «다음 칸 이름»을 자른다. 이것이 없으면 주소에 라벨이 붙어 저장된다. */
    function addrClean(s) {
      var t = String(s || '').replace(/[|]/g, ' ').replace(/\s+/g, ' ').trim();
      t = t.replace(/\s*(?:본\s*점\s*소\s*재\s*지|사\s*업\s*장\s*소\s*재\s*지|소\s*재\s*지|사\s*업\s*의?\s*종\s*류|업\s*태|종\s*목|개\s*업|발\s*급|교\s*부|전\s*화|법\s*인|대\s*표|수\s*입|사\s*업\s*자)[\s\S]*$/, '');
      t = t.replace(/^[가-힣\s]*(?:소\s*재\s*지|주\s*소)\s*[|:：]?\s*/, '');
      t = t.replace(/^\d{5}\s*/, '').replace(/^[:：|,\s]+/, '');
      return t.replace(/\s+/g, ' ').trim();
    }

    /* ⑥-㉮ 라벨이 붙은 «줄»에서 — 여기가 가장 정확하다.
       ⚠ 「사업장」이 적힌 줄을 **먼저** 고른다. 법인 등록증은 사업장·본점 두 줄인데,
         업체관리가 쓰는 것은 «사업장» 쪽이다. 앞줄부터 훑으면 대개 맞지만, 스캔이
         줄 차례를 뒤집는 일이 있어 그때도 틀리지 않게 두 번 훑는다. */
    var addrLineRe = /(?:사\s*업\s*장\s*(?:소\s*재\s*지|주\s*소)|소\s*재\s*지|주\s*소)\s*[:|：]?\s*(.+)$/;
    var 사업장먼저 = function (want) {
      for (var n = 0; n < lines.length; n++) {
        if (want && !/사\s*업\s*장/.test(lines[n])) continue;
        if (!want && /본\s*점/.test(lines[n])) continue;   // 본점은 마지막에만
        var mm = lines[n].match(addrLineRe);
        if (!mm) continue;
        var vv = addrClean(mm[1]);
        if (vv.length > 5) return vv;
      }
      return '';
    };
    out.address = 사업장먼저(true) || 사업장먼저(false) || '';

    /* ⑥-㉯ 줄에서 못 찾았으면 예전 규칙으로 — 이어 붙인 글에서 찾는다.
       ⚠ 담는 글자에 «콜론»을 넣었다. 그것이 빠져 있어서 본점 줄이 있는 등록증이
         통째로 실패했다. 꼬리는 addrClean 이 자른다. */
    if (!out.address) {
      var addrRe = [
        new RegExp('(?:사\\s*업\\s*장\\s*소\\s*재\\s*지|소\\s*재\\s*지|사\\s*업\\s*장\\s*주\\s*소)\\s*[:：]?\\s*([가-힣A-Za-z0-9\\s,·\\-()\\[\\]:：]{5,200}?)(?=\\s*(?:업\\s*태|종\\s*목|개\\s*업|전\\s*화|법\\s*인|발\\s*급|수\\s*입|대\\s*표|$))'),
        new RegExp('((?:' + SIDO + ')[특별시광역도자치]*\\s*[가-힣A-Za-z0-9\\s,·\\-()]{10,200})')
      ];
    ...
    /* ⑥-㉰ 라벨이 아예 안 읽혔으면 «시·도로 시작하는 줄»을 쓴다. */
    if (!out.address) {
      var sidoRe = new RegExp('(?:' + SIDO + ')');
      for (i = 0; i < lines.length; i++) {
        if (sidoRe.test(lines[i])
            && lines[i].length >= 10 && lines[i].length <= 200
            && !/대표|사업자|등록번호|업태|종목/.test(lines[i])) {
          v = addrClean(lines[i]);
          if (v.length > 5) { out.address = v; break; }
        }
      }
    }
  ...
  function autoOk(result, nowMs) {
    ...
    if (r.kind === 'bizreg' || r.kind === 'sme') {
      ...
      /* ★★ 국세청에 «없는» 번호는 여기서 막는다 — 체크섬은 통과하기 때문이다.
         이 문이 없어서 123-45-67891 이 기업정보함까지 들어갔다(2026-09-15 대표 보고).

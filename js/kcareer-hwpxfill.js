...
(function (root) {
  ...
  var FIELD_LABELS = [
    ...
    /* ★ 「노무법인」 표 — 사람이 아니라 «우리 법인» 것이다 (대표 지시 2026-09-13
       ...
       ⚠ 「대표자명」(→ name)과 갈라 둔다 — 그쪽은 «내는 사람»이고 여기는 «법인의 대표»다.
         둘이 같은 사람일 때가 많지만 직원이 낼 때는 다르다.
       ⚠ 값은 _cvFillData 가 이알피 법인정보(data/company_info)에서 가져온다 — 여기 베껴 두지 말 것. */
  ...
  var COL_LABELS = [
    ...
    /* ⚠ 「종류」 한 낱말을 빠뜨려 대표 이력서의 자격 표 머리줄을 못 알아봤다(2026-09-13).
       머리줄로 안 보이면 위쪽 학력 목록이 그 아래 빈 줄까지 흘러 들어간다. */
    { re: /^(자격증명|자격명|면허명|자격사항|자격증|면허|자격종류|종목|자격종목|종류|구분종류)$/, key: 'certName' },
    /* ⚠★ 「취득«년»월일」이 빠져 있었다 — 사전엔 「취득«연»월일」만 있었다.
       한 글자 차이로 대표 서식의 그 칸을 영영 못 알아봤다(실측 2026-09-13). */
    { re: /^(취득년도|취득일자?|발급일자?|취득연월일|취득년월일|취득연월|취득년월|자격취득일|발행일자?|수여일자?|발급연월일)$/, key: 'gotAt' },
    /* ★ 상벌 표 — 이력서에 거의 늘 있는데 사전에 한 낱말도 없었다.
       ⚠ 이 열쇠들은 «채우지 않는다»(LIST_FILL_KEYS 에 없다). 머리줄로 알아보아
         «경계»로 삼기 위한 것이다 — 자격증·표창을 실제로 써 넣을지는 따로 정할 일이다. */
    { re: /^(상벌사항|상벌내용|포상사항|상훈사항|징계사항|상벌)$/, key: 'awardWhat' },
    { re: /^(상벌기관|수여기관|포상기관|상훈기관|시행기관장)$/, key: 'awardOrg' },
    { re: /^(성적|성적등급|등급|점수|취득점수|성적점수)$/, key: 'grade' },
     ...
     ⚠★ 이 자를 넓히는 것은 «덮어쓰기»를 넓히는 것이다. 잘못 넓히면 대표가 손으로
       적어 둔 값을 지운다 — 서류가 «조용히» 틀린다. 그래서 다음을 지킨다:
         ...
         ② 한글·숫자가 «뜻을 이루면» 자리표가 아니다.
            ...
            1980.01.01 은 «말이 되는 날짜»라 자리표가 아니다 — 절대 안 덮는다.
    ...
    hits.forEach(function (L) {
      ...
      out = out.replace(/(<hp:t(?:\s[^>]*)?>)([\s\S]*?)(<\/hp:t>)/g, function (m, a, inner, b) {
        ...
        /* 밑줄·넓은 공백은 «값 자리»이므로 삼켜서 값으로 바꾼다(안 삼키면
           「직장:041-000-0001_______」처럼 밑줄이 남아 줄이 넘친다).

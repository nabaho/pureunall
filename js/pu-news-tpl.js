  ...
  function 기사줄(항목들, 그림, 폭) {
    ...
    var 줄 = 것.map(function (x) {
      ...
      var 몸 = 내글
        ...
        : esc(x.제목 || '');
      /* ★★ 기사에는 «원문 링크를 안 붙인다» (대표 지시 2026-09-18
           「원문 클릭 매일노동뉴스로 가지말고 그냥 정리된 내용으로 만들 면 안되나?」).
         ⚠ 까닭 둘이다. ① 매일노동뉴스는 «유료 구독»이라 받으신 분이 눌러도 대개
           담벼락을 만난다 — 눌러도 못 보는 손잡이는 없느니만 못하다.
           ② 읽는 자리가 우리 편지에서 남의 쪽으로 튄다.
         ★ 실을 것은 «우리가 쓴 글»뿐이고, 그것은 이미 여기 다 있다.
         ⚠⚠ «출처 이름»은 남긴다 — 꼭지 아래 「참고: 매일노동뉴스」로 붙는다.
           참고한 것을 안 밝히는 것이 링크가 없는 것보다 훨씬 나쁘다.
         ⚠ 판례·자료·법령의 링크는 그대로다 — 법제처·고용노동부는 누구나 열 수 있다. */
      /* ★★★ 되돌렸다 (대표 지시 2026-09-20 「주간노동뉴스는 원문을 붙여라 …
             검토필요없이 자동으로 원문붙여서 넣자」).
         ★ 우리가 쓴 글이 «없는» 줄에는 원문 링크를 붙인다 — 제목만 덜렁 두고 갈 데가
           없으면 받는 분이 더 알아볼 길이 없다. 「원문을 붙여라」가 이 뜻이다.
         ⚠ 우리가 «쓴» 줄에는 그대로 안 붙인다(2026-09-18 지시 그대로) — 읽을 것이
           편지 안에 다 있는데 남의 쪽으로 튀게 할 까닭이 없고, 유료 담벼락도 그대로다. */
      var 링 = 내글 ? '' : (href(x.링크)
    ...
    Core.꼭지들.forEach(function (g) {
      ...
      것.forEach(function (x) {
        if (x.갈래 === '자료') {
          ...
          /* ★★ 기사는 «우리 말»이 본문이다 — 신문 제목을 옮기지 않는다.
             ⚠⚠ 여기가 서식 쪽(기사줄)과 «달랐다»(2026-09-18 에 알았다). 서식은
               2026-09-08 부터 우리 글을 싣는데, 평문은 여태 «신문 제목 + 원문 링크»를
               싣고 있었다. 서식을 못 읽는 프로그램으로 받으신 분에게는 그동안
               남의 제목이 그대로 갔다는 뜻이다. 한쪽만 고치면 이런 일이 생긴다.
             ⚠ 원문 링크는 안 싣는다(대표 지시 2026-09-18) — 출처 이름은 남긴다. */

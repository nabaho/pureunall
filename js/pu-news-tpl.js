...
(function (global) {
  ...
  /* ── 요약판 머리 — 「신문」 (대표 결정 2026-09-17) ───────────────────────
     ⚠ 요약판«만» 이것을 쓴다. 전문 보기 쪽은 큰 표지와 차림표를 그대로 둔다 —
       거기서는 차림표가 «붙잡히는 줄»이라 실제로 일을 한다(news-view.js).

     ★ 왜 바꿨나 — 대표 지시 2026-09-17 「좀더 세련되게 … 화면을 너무 아래로 안
       내려오게」. 재 보니 내용이 시작되기 «전에» 400px 을 쓰고 있었다:
         큰 사진 띠 184 + 차림표 85 + 머리·안내 134.
       그중 차림표는 메일에서 «눌러도 아무 데도 안 간다»(#자리이동을 무시한다).
       장식 하나가 한 화면의 1/8 을 먹고 있었다.
     ⚠ 3px·2px 줄은 «칸에 색을 깔아» 만든다. div 테두리는 아웃룩에서 사라진다. */
  function _줄띠(높이, 빛깔) {
    return '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">'
      + '<tr><td style="height:' + 높이 + 'px;line-height:' + 높이 + 'px;font-size:1px;'
  ...
  /* 요약판 꼭지 머리 — 딱지를 걷고 «글자»로 세운다.
     ⚠ 껍데기(<tr><td>) 없이 돌려준다. 두 칸 안에 바로 들어가기 때문이다. */
  function 요약꼭지제목(g) {
    var 딱 = String(g.딱지 || '').toUpperCase();
    return '<a id="g-' + esc(g.키 || '') + '"></a>'
      + (딱 ? '<div style="font-size:11px;font-weight:bold;color:' + 색.딱지 + ';'
        + 'letter-spacing:2px;line-height:1.4;font-family:' + 폰트 + ';">' + esc(딱) + '</div>' : '')
      + '<div style="font-size:17px;font-weight:bold;color:' + 색.짙은갈 + ';line-height:1.4;'
      + 'font-family:' + 폰트 + ';">' + esc(g.이름) + '</div>'
  ...
  function 기사줄(항목들, 그림, 폭) {
    ...
    var 줄 = 것.map(function (x) {
      ...
      var 몸 = 내글
        ...
        : esc(x.제목 || '');
      /* ★★★ 2026-09-18 「원문 클릭 매일노동뉴스로 가지말고」로 우리가 쓴 줄에서
             링크를 걷었었고, 2026-09-20 「원문을 붙여라」로 «안 쓴 줄»에만 되살렸다.
         ⚠⚠ 그런데 대표께서 화면으로 직접 보시고 다시 짚으셨다(2026-09-20
             「주간노동뉴스는 왜 팝업도 없고 기사도 없나 출처도 각각 사라졌다」) —
           우리가 쓴 줄만 «출처도 링크도 없는 밋밋한 문단»이 되어 있었다. 아래 세
           줄(대유위니아그룹·아웃소싱 이직률 …)이 다 우리 글이 되자 실제로 그렇게
           보였다. 「오로지 주간노동뉴스만 원문으로 해라」는 «꼭지 사이»의 구분이지,
           같은 꼭지 «안에서» 쓴 줄과 안 쓴 줄을 가르라는 말이 아니었다.
         ★ 그래서 «쓴 줄이든 안 쓴 줄이든 똑같이» 출처와 원문 링크를 단다.
           자료·판례의 링크는 그대로다 — 법제처·고용노동부는 누구나 열 수 있다. */
      var 링 = href(x.링크)
        ? ' <a href="' + href(x.링크) + '"' + 새탭 + ' style="color:' + 색.남색
          + ';font-size:12px;text-decoration:none;font-weight:bold;">원문 ↗</a>'
        : '';
      var 출처칸 = x.언론사
        ? ' <span style="color:' + 색.흐린글 + ';font-size:12px;">· ' + esc(x.언론사) + '</span>'
        : '';
    ...
    /* ── 「위 정리는 푸른노무법인이 썼습니다」 ──
       ⚠⚠ 2026-09-20 전에는 여기서 «참고: 매일노동뉴스 · …」로 출처를 «한데 모아»
         적었다 — 줄마다 언론사가 없던(우리 말이 있으면 안 적던) 시절의 자리다.
         이제 위에서 줄«마다» 출처를 단다(출처칸). 여기서 또 모아 적으면 같은
         이름이 두 번(줄에 한 번, 여기 또 한 번) 나온다.
       ★ 그래서 이 줄은 «누가 정리했는지»(저작권을 위한 주체 표시)만 남긴다.
         이름 목록은 지운다 — 지운 것이 아니라 «자리를 옮겼다»(줄마다). */
    var 씀 = 것.some(function (x) { return String(x.우리말 || '').trim(); });
  ...
  function 자료카드(x) {
    ...
    /* ★★★ 제목을 외부 상세 페이지로 링크하지 않는다 (대표 지시 2026-09-20
         「팝업창으로 바로 가지마라 고용노동정책 지원기업은 이럴경우 불필요하다」).
       ⚠⚠ 정책 가이드는 이미 카드 안에 제목·발행처·(있으면) 우리말/목차·내려받기가
         다 있다. 제목을 눌러 외부 사이트(고용노동부·기업마당)로 «새로 뛰어나가는»
         것은 다운로드 말고는 얻을 것이 없는데 창만 하나 더 연다 — 대표께서
         실제로 눌러 보시고 「이럴 필요가 없다」고 짚으셨다.
       ★ 「오로지 주간노동뉴스만 원문으로 해라」의 다른 절반이 이것이다 — 뉴스가
         아닌 자료·판례에는 «클릭해서 나가는 링크»를 두지 않는다.
       ⚠ 링크(x.링크) 자체는 지우지 않는다 — 창고 자료를 찾거나 겹침을 가릴 때
         쓰는 값이다. 여기, «편지에 그릴 때»만 안 쓴다. */
    var 제목칸 = 제;
  ...
  function 요약칸(항목들, 꼭지, 웹주소, 우리글) {
    ...
    (항목들 || []).forEach(function (x) {
      ...
      if (!글) return;
      /* ★★ 「내려받을 것이 있다」를 요약에서 «표시»한다 (대표 지시 2026-09-14
           「내려받기 다운로드 있을경우 이부분 표시해라 그렇게 해야 클릭해서 찾는다」).
         ⚠ 이것이 없으면 공고문·서식이 딸린 줄과 안 딸린 줄이 «똑같이» 보인다.
           손님은 눌러 봐야 알고, 대개 안 눌러 본다 — 그대로 묻힌다.
         ⚠⚠ 파일이 «있을 때만» 붙인다. 없는데 붙이면 눌러도 받을 것이 없고,
           한 번 그러면 다음부터 이 표시를 안 믿는다 — 붙인 뜻이 통째로 없어진다. */
      줄.push({ 글: 글, 곁: _요약곁(x), 자리: x._자리 || '', 받기: !!href(x.파일) });
    });
    if (!줄.length) return '';
    ...
    var 몸 = 줄.map(function (r) {
      ...
      var 속 = u
        ...
        : esc(r.글);
      /* ★ 점을 「—」로 (대표 결정 2026-09-17 「신문 안」). 가는 가로줄이 글줄과 같은
           결이라 목록이 조용해지고, 줄 간격을 1.7 → 1.6 으로 좁혀도 안 답답하다. */
      return '<tr>'
        + '<td valign="top" width="12" style="width:12px;padding:5px 0 5px 0;'
        + 'font-size:14px;line-height:1.6;color:#c9bfb2;font-family:' + 폰트 + ';">&#8212;</td>'
        + '<td style="padding:5px 0 5px 6px;font-size:14px;line-height:1.6;color:' + 색.글 + ';'
  ...
  /* 맨 위 띠 — 「전문은 여기서」.
     ★ 상자를 걷고 «한 줄»로 (대표 결정 2026-09-17 「신문 안」). 상자가 60px 을 먹었는데,
       말하는 것은 한 줄이다. 요약만 보고 끝내실 분께도 이 한 줄이면 닿는다. */
  function 전문보기띠(웹주소) {
    var u = href(웹주소 || '');
    if (!u) return '';
    return '<tr><td style="padding:10px ' + 옆여백 + 'px 0 ' + 옆여백 + 'px;">'
  ...
  /* 업체 주소에 맞는 지역 소식만 통마다 끼운다. 민간 기사 원문은 받지 않고
     ...
     ⚠ 여기 링크는 «지자체·공공기관 공고»다(등록할 때 go.kr 만 받는다). 신청 마감 같은
       것이라 받는 분이 꼭 열어야 한다 — 신문 기사 링크를 걷은 것과 다른 자리다. */
  ...
  function 꼬리(설정, 범위, 더한것) {
    ...
    return '<tr><td style="padding:20px ' + 옆여백 + 'px 0 ' + 옆여백 + 'px;">'
      ...
      + 'font-family:' + 폰트 + ';">'
      /* ⚠⚠ 이 줄은 «약속»이다. 편지가 하는 일과 어긋나면 안 된다 —
           2026-09-18 에 기사 원문 링크를 걷으면서 「원문 링크는 함께 밝힙니다」를
           함께 고쳤다. 어긋난 안내는 없는 것보다 나쁘다. */
      /* ⚠⚠ 2026-09-20 에 또 고쳤다. 기사에 원문 제목·링크가 다시 실리므로
             「직접 정리한 글«만» 싣습니다」가 거짓이 된다. */
      + '기사는 <b>본문을 옮기지 않고</b> 제목과 원문 링크까지만 싣습니다. '
      + '푸른노무법인이 정리한 글이 있는 꼭지는 <b>그 글</b>이 실립니다. '
      + '어느 매체를 참고했는지는 꼭지마다 밝힙니다.<br><br>'
  ...
  function 평문짓기(회차자료, 설정, 받는지역) {
    ...
    Core.꼭지들.forEach(function (g) {
      ...
      것.forEach(function (x) {
        if (x.갈래 === '자료') {
          ...
          /* ★★ 기사는 «우리 말»이 있으면 그것이 본문이다 — 신문 제목을 대신 옮기지
               않는다. 우리 말이 없으면 제목이 그 자리를 대신한다.
             ⚠⚠ 서식 쪽(기사줄)과 «반드시 같아야» 한다 — 2026-09-18 에 한쪽만 고쳐서
               서식을 못 읽는 프로그램으로 받으신 분에게 다른 편지가 간 적이 있다.
             ★★★ 2026-09-20 대표 화면 지적 「주간노동뉴스는 왜 팝업도 없고 기사도
               없나 출처도 각각 사라졌다」— 쓴 줄만 출처·링크가 없는 밋밋한 문단이었다.
               «쓴 줄이든 안 쓴 줄이든 똑같이» 출처와 원문 링크를 단다. */
          var 내말 = String(x.우리말 || '').trim();
          줄.push('· ' + (내말 || x.제목 || '') + (x.언론사 ? ' · ' + x.언론사 : ''));
          if (x.링크) 줄.push('  ' + x.링크);
        }
      });
      줄.push('');
    ...
    줄.push('---');
    /* ⚠⚠ 서식 쪽 꼬리와 «같은 약속»이어야 한다. 2026-09-18 에 서식만 고치고
         여기를 잊어, 서식을 못 읽는 프로그램으로 받으신 분에게는 「원문 링크는
         함께 밝힙니다」가 그대로 갔다 — 검사가 그 자리에서 잡았다. */
    줄.push('기사는 본문을 옮기지 않고 제목과 원문 링크까지만 싣습니다. '
      + '푸른노무법인이 정리한 글이 있는 꼭지는 그 글이 실립니다. '
      + '어느 매체를 참고했는지는 꼭지마다 밝힙니다.');

/* 급여담당자 연락처 엑셀 → 업체관리 (대표 결정 2026-08-24)
   ...
   그래서 「다온케이(가나-새롬요양센터)」처럼 한 업체의 여러 사업장이
...
(function (root) {
  ...
  /* 앞머리 — 괄호 앞까지. 「다온케이(가나-새롬요양센터)」 → 「다온케이」 */
  function stemName(v) {
  ...
  function parseGrid(grid, opt) {
    ...
    for (var r2 = hi + 1; r2 < rows.length; r2++) {
      ...
      /* ★ 사업장명이 빈 줄 = «윗줄 사업장의 둘째 담당자» (예: 「(주)나루유」의
  ...
  /* ── 빈칸 물려받기 (대표 결정 2026-08-24) ──
     「새별반찬(모종점)」에만 연락처가 있고 「새별반찬(배방점)」은 비어 있다 —
     한 곳이 지점을 여럿 둔 것이라 위 줄과 같은 사람이다.
     ⚠ **이름 앞머리가 같을 때만** 물려받는다. 「라온정밀」 다음의
     「마바이엔지」까지 물려받으면 남의 연락처가 붙는다. */

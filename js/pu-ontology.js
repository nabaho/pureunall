...
(function(root, factory){
  ...
  /* 사람이 적은 글에서 «몸통만» 뽑는다. 머리는 버린다 — 옛 번호로도 찾히게.
     ⚠⚠ 「끝에 붙은 다섯 자리」로 찾으면 안 된다 — 사업자번호 123-81-20079 의
  ...
  var INFRA_ROOTS = {
    ...
    /* 이번 달 판독 요금 한도·경고선·단가 (대표 결정 2026-09-10 · ₩30,000 / ₩25,000).
       ⚠ 업무 자료가 아니라 «정책 숫자»다 — 담긴 것은 숫자 셋뿐이라 직원 누구나
         읽는다(사진첩 판독 띠가 「한도를 다 썼습니다」라고 말하려면 읽어야 한다).
         고치는 것은 총괄 관리자만 — 아무나 올릴 수 있으면 한도가 아니다. */
    ai_read_budget:'판독 요금 한도 — 이번 달 한도·경고선·단가(숫자 셋)',
    /* 포털 건의함 — 직원이 대표께 올린 글이다. **업무 개체로 만들지 않는다.**
       담긴 것이 «의뢰인 실명·사건번호가 섞인 자유 글»이라, 색인에 넣는 순간
       원본보다 넓은 권한으로 개인정보가 퍼진다(CLAUDE.md 「관계 진단은 경량
       메타데이터만 읽는다」·「원본보다 넓은 권한으로 공개하지 않는다」).
       그래서 «자리만» 밝히고 관계로 엮지 않는다.
       ⚠ 원문은 총괄관리자만 읽는다(규칙 suggestions_private).
    ...
    /* 같은 까닭 — 안내문(2026-09-23)에 붙이는 PDF·한글 원본. 문서 «본문»이라 진단에 넣지 않는다. */
    newsletter_files:'창고(Storage) — 안내문 첨부 문서. DB 자리 아님, 주인은 뉴스레터(news)',

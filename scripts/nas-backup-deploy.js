...
/* ★ 열쇠를 «새것으로 갈아 끼운다» (2026-09-20 대표 지시 「지금 바꾼다」)
   ── 왜 있나
     열쇠가 채팅 화면에 한 번 찍히면 그 열쇠는 더 이상 우리만 아는 것이 아니다.
     그런데 평소 흐름은 「이미 있으면 그대로 쓴다」라서 바꿀 길이 없었다.
   ⚠ 갈아 끼운 뒤 «함수를 다시 올려야» 옛 열쇠가 죽는다 — 함수는 올릴 때의 판을
     물고 돌기 때문이다. 이 스크립트는 열쇠를 넣은 «뒤»에 올리므로 그 차례가 맞다.
   ⚠ --deploy 없이는 아무것도 안 바꾼다(보여만 준다). */
const 새열쇠강제 = process.argv.includes('--new-key');
...
function 열쇠준비() {
  ...
  if (기존값 && !새열쇠강제) {
    ...
    말('   (열쇠를 «새것으로 갈아 끼우려면» 뒤에 --new-key 를 붙여 다시 돌리세요)');
    return 기존값;
  }

  const 새열쇠 = crypto.randomBytes(32).toString('base64url');
  if (!올린다) {
    말(기존값 ? '② 열쇠 — 올릴 때 «새것으로 갈아 끼웁니다»(--new-key).'
             : '② 열쇠 — 아직 없습니다. 올릴 때 새로 만들어 넣습니다.');
    return '(올릴 때 만듭니다)';
  }
  if (기존값) {
    /* ⚠ 옛 열쇠는 «함수를 다시 올리는 순간» 죽는다 — 함수가 올릴 때의 판을 물고 돌기 때문이다.
       그래서 여기서 넣고, 바로 뒤 올리기()가 그것을 굳힌다. 그 차례가 뒤바뀌면 안 된다. */
    말('② 열쇠 — 이미 있지만 «새것으로 갈아 끼웁니다»(--new-key).');
    말('   옛 열쇠는 곧 이어지는 ③ 올리기가 끝나는 순간 못 쓰게 됩니다.');
  }
...
(async function () {
  ...
  if (열쇠 && 열쇠 !== '(올릴 때 만듭니다)') {
    ...
    말('');
    /* ⚠★ 이 줄 아래에 «진짜 열쇠»가 찍힌다. 그래서 바로 위에 한 번 더 못을 박는다 —
       2026-09-19 에 이 글이 그대로 채팅에 붙어 열쇠가 한 번 드러났다. 글이 길어
       아래쪽 경고는 스크롤 밖으로 밀려 안 읽힌다. 위험한 것 «바로 앞»에 적는다. */
    말('  ⚠⚠ 아래는 «열쇠가 든» 글입니다 — 나스 칸에만 넣으시고,');
    말('     채팅·메일·메모 어디에도 붙여넣지 마세요.');
    말('────────────────────────────────────────────────────────────');
    말(완성);
    말('────────────────────────────────────────────────────────────');
  } else {

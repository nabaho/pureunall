...
(async function () {
  ...
  if (열쇠 && 열쇠 !== '(올릴 때 만듭니다)') {
    ...
    const 됐나 = 클립보드에(완성);
    /* ⚠★★ 클립보드만 믿지 않는다 (2026-09-19 대표 화면 — 「눌러도 전혀 반응이 없다」)
       ── 무엇이 있었나
         clip 이 성공했다고 여기서 말했고, 대표님도 그 말을 믿고 나스로 건너가셨다.
         그런데 그 사이(단추 여러 번 클릭·창 여러 개 오가는 몇 분) 클립보드가 다른
         것으로 덮였다 — Ctrl+V 를 눌러도 아무 반응이 없었다. 클립보드는 «지금 이 순간»만
         보장하지, 몇 분 뒤까지 보장하지 않는다. 그런데 이 창엔 그 값이 다시 없었다 —
         화면에 한 번도 안 찍었기 때문이다.
       ── 그래서
         복사가 «성공했다고 나와도» 화면에 «그대로» 한 번 더 찍는다. 클립보드가 지워지면
         이 창을 위로 스크롤해서 마우스로 긁어 복사하면 된다 — 다시 돌릴 필요가 없다. */
    if (됐나) {
      ...
      말('  ⚠ 나중에 Ctrl+V 가 안 되면(클립보드가 다른 것으로 덮였을 때) — 다시 돌리지 말고');
      말('    이 창을 «위로 스크롤»해서 아래 글을 마우스로 긁어 복사하세요:');
    } else {
      말('  clip 이 안 됩니다 — 아래를 마우스로 긁어 나스 「사용자 정의 스크립트」 칸에 붙여넣으세요:');
    }
    말('');
    말('────────────────────────────────────────────────────────────');
    말(완성);

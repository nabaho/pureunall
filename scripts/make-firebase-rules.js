...
rules.data = {
  ...
  /* 사진첩에서 「담당자 고르고 계약 만들기」를 누르면 남는 «요청 한 줄» (2026-09-19).
     계약관리가 열릴 때 읽어 검사하고, 만들면 지운다(걸린 것은 까닭을 적어 남긴다).
     ⚠ 계약과 «같은 권한»이다 — 담기는 것이 계약에 들어갈 회사 정보 그대로다.
       여기만 더 열면 계약보다 넓게 새는 구멍이 된다. */
  contract_requests: { '.read': LOGIN, '.write': LOGIN },
  /* 계약서 양식(본문·첨부 원본)과 «지운 기본 양식 번호» (2026-09-26 이름을 붙임).
     여태 이름 없이 $other 로 떨어져 있었다 — 이알피가 글자로 된 경로 없이 dbGet 으로만 불러
     검사 눈에 안 띄었다. 양식 화면을 문서관리로 옮기며 경로가 글자로 드러났다.
     ★ 권한은 «그대로»(재직 직원 전원) — 이알피 「계약서 출력」을 직원도 쓰기 때문이다.
  ...
  /* 근태·일정 — 여태 이름이 없어 $other 로 떨어져 있었다.
     ★ 권한은 «지금 그대로»다(재직 직원 전원 읽기·쓰기). 이름만 적는다 — 동작은 한 톨도 안 바뀐다.
     이름을 붙이는 까닭: 푸른 캘린더가 이 자리를 «이름 그대로» 쓰게 되면서,
     무엇이 어디에 열려 있는지 세어 볼 수 있어야 하기 때문이다(검사가 그것을 본다).
     ⚠ 근태는 급여로 흘러간다 — 좁힐지는 대표 판단이다. 지금 누가 고칠 수 있는지:
       직원 누구나(제 것도 남의 것도). 마감된 달만 앱이 막는다(서버 규칙이 아니라 앱이).
     ⚠ 마감 자물쇠를 서버 규칙으로 옮기는 것은 따로 볼 일이다 — 지금 옮기면
       마감 해제·일괄 정리 같은 관리자 길이 함께 막힌다. */
  attendance_records: { '.read': LOGIN, '.write': LOGIN },   /* 연차·반차·출장·이음 근무 */
  my_schedules:       { '.read': LOGIN, '.write': LOGIN },   /* 일정 */
  presence_hours: { '.read': LOGIN, '.write': LOGIN },   /* 근무 시간 */
  /* 우리 법인 자신의 정보 — 법인명·대표·사업자번호·법인등록번호·사무소 주소·전화·팩스·직인.
     2026-09-13 경력관리가 기관 서식의 「노무법인」 칸을 채우려고 «읽기» 시작하면서 이름을 붙였다.
     ★ 권한은 «지금 그대로»다(여태 $other 로 떨어져 재직 직원 전원이었다) — 이름만 적는다.
  ...
  /* ★★ 메뉴 접근 권한표(사번 → 볼 수 있는 메뉴 목록) — 2026-09-21
     ⚠⚠ 이름이 없어 $other 로 떨어져 «재직 직원 누구나 고칠 수» 있었다.
       이 표가 isMenuPermitted() 가 보는 «진짜 자리»다 — 제 사번 칸에 메뉴를 적어 넣으면
       그 메뉴가 열린다. 스스로 권한을 키울 수 있는 길이었다.
     ★ 읽기는 «좁히지 않는다». 앱이 켜질 때 이 표를 통째로 받아 제 메뉴를 고른다 —
       막으면 아무에게도 메뉴가 안 뜬다. (남의 메뉴 권한이 보이는 것은 비밀이 아니다.)
     ★ 쓰기는 관리자·위임관리인만. 고치는 화면이 셋인데(환경설정 ▸ 데이터 접근,
       환경설정 ▸ 사용자 삭제) 모두 환경설정 안이고, 지금 그 문을 지나는 사람은
       관리자 1명·위임관리인 1명뿐이다(2026-09-21 실측). 그 둘은 그대로 쓴다.
     ⚠ 더 좁히려면(관리자만) 위임관리인이 환경설정을 열 때 조용히 실패한다 —
       화면도 함께 고쳐야 하는 별개의 일이다. 대표 판단 몫으로 남긴다. */
  security_perms:  { '.read': LOGIN, '.write': MGR },

  /* ★★ 구글 Vision 열쇠 — 2026-09-21
     ⚠ 이름이 없어 $other 로 떨어져 «재직 직원 누구나 읽을 수» 있었다. 열쇠다.
     ★ 지금 이 자리를 읽는 코드는 «하나도 없다»(온 저장소를 훑어 확인).
       판독은 서버 대리인(readVision)이 하고, 경력관리는 제 localStorage 를 쓴다.
       남은 것은 브라우저에서 부르던 시절의 찌꺼기다.
     ⚠ 그래도 «지우지 않는다» — 지우는 것은 되돌릴 수 없고, 내가 못 보는 데서
       쓰고 있을 수 있다. 문만 잠그고 대표께 «콘솔에서 폐기»를 권한다
       (누가 읽어 갔는지는 알 길이 없으므로 잠그는 것만으로는 부족하다). */
  setting_google_vision_api_key: { '.read': ADMIN, '.write': ADMIN },

  /* 구글 계정(일정을 만든 메일) ↔ 직원 사번 — 온 회사가 같이 보는 «이름표»다.
     ⚠ 이름이 없어 2026-09-20 까지 $other 로 떨어져 있었다(재직자 누구나 쓰기).
       한 사람이 잘못 이으면 그 뒤로 모두의 달력에서 «남의 이름·남의 색»이 뜬다 —
       고쳐 준 사람은 자기 화면만 보므로 한참 모른다. staff_colors 와 같은 결이라
       같은 권한으로 맞춘다: 읽기는 직원 누구나(색·거르기에 쓴다), 쓰기는 관리자·위임관리인.
     ⚠ 담기는 것은 메일과 사번뿐이다 — 자물쇠도, 개인 자료도 아니다.
       (푸른 캘린더 pu-cal.html · 이알피 법인대시보드 둘 다 이 자리 하나를 쓴다) */
  gcal_mail_sid:   { '.read': LOGIN, '.write': MGR },

  /* 이음센터 내부 직원 «비고»(사번 → 요구사항 한 줄) — 「2·4주 금요일만」 같은 것.
     ⚠ 이름이 없어 2026-09-20 까지 $other 로 떨어져 있었다. 권한은 «넓히지도 좁히지도»
       않는다 — 이알피에서 전 직원이 고치던 자리이고 그것이 확정 사양이다(그대로 LOGIN).
       이름을 적는 까닭은 권한을 바꾸려는 것이 아니라, 다음에 $other 를 좁힐 때
       이 자리가 «누구 것인지 모른 채» 함께 잠기지 않게 하려는 것이다.
     ⚠ 외부 인원의 비고는 여기가 아니다 — external_staff 레코드의 note 칸이다. */
  ieum_notes:      { '.read': LOGIN, '.write': LOGIN },

  /* 푸른 캘린더가 «보던 자리»를 담는 곳 (대표 지시 2026-09-18, 앱을 따로 뺌).
     담기는 것은 어느 탭·어느 달·어느 거르개였나뿐이다 — 업무 자료가 아니다.
     ⚠ 제 칸만 쓴다(uid 로 가른다). 남의 «보던 자리»를 고칠 까닭이 없다.
     ⚠ 읽기도 제 것만이다 — 누가 무엇을 보고 있었는지는 남이 알 일이 아니다. */
  cal_view: { '$uid': { '.read': "auth != null && auth.uid == $uid",
...
/* ══ 전자서명 ══════════════════════════════════════════════════════════
   ...
   ※ 같은 것을 규칙 올리개(scripts/rules-deploy.js)도 따로 찾았다 — 이 줄이 만들개에
     없던 동안 「콘솔에만 있는 규칙이 사라진다」며 멈춰 세웠다. 안전장치가 맞물린 자리다. */

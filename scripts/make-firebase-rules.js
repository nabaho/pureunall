...
/* ══ 전자서명 ══════════════════════════════════════════════════════════
   ⚠ 근로자는 링크로 들어와 «암호화된 채로» 낸다. meta 는 그래서 열려 있다.
     낸 것은 우리 직원만 읽는다. */
rules.esign = { cases: {
  /* ★ 2026-09-20 — 이 한 줄은 «콘솔에 이미 살아 있던 것»이다. 만들개에는 없어서
     올리개가 「사라진다」며 멈췄다(안전장치가 제 일을 했다). 누군가 콘솔에서 손으로
     더한 것으로 보인다 — 사건 «목록»을 받으려면 cases 자리에서 읽을 수 있어야 한다
     ( 안쪽만 열려 있으면 한 건씩은 읽혀도 목록은 못 받는다).
     ⚠ 지금 상태를 그대로 옮겨 적은 것이다 — 넓히지 않았다.
     ⚠ 다만 읽기는 «아래로 흐른다» — 이 줄이 있으면 우리 직원(MAIL)은 secret·arrears·
       submissions 까지 다 읽는다. 그것이 이미 지금의 상태다. 좁히려면 목록용 색인을
       따로 두고 이 줄을 빼야 하는데, 그건 대표님 판단이 필요한 «권한 줄이기»라
       여기서 조용히 하지 않는다. */
  '.read': MAIL,
  $caseId: {

...
/* ══════════════════════════════════════════════════════════════════════════
   안내문 첨부 한 개 — ?file=<열쇠>&n=<이름> (대표 지시 2026-09-23 안내문)
   ══════════════════════════════════════════════════════════════════════════
   그림(?img=)과 같은 문·같은 잣대다. 다른 것은 셋:
     ① 자리가 newsletter_files/ 다 — 사진 칸(okImage)과 규칙이 달라 자리를 가른다.
     ② «내려받게» 준다(attachment) — 우리 주소 안에서 문서가 «열리면» 안 된다.
        그래서 종류도 가리지 않고 octet-stream 으로 준다(nosniff 와 함께).
     ③ 이름(n)은 «내려받을 파일 이름»에만 쓴다 — 어느 파일인지는 열쇠만 정한다.
   ⚠ 받는 확장자는 문서·사진뿐이다. html·svg·js·zip 은 안 받는다 —
     앞의 셋은 브라우저가 스크립트로 읽고, zip 은 속을 알 수 없다. */
const 파일끝들 = ['pdf', 'hwp', 'hwpx', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'jpg', 'png'];
function 파일열쇠(v, 이름) {
  const s = String(v == null ? '' : v);
  const m = /^([0-9a-f]{32})\.([a-z]{3,4})$/.exec(s);
  if (!m || 파일끝들.indexOf(m[2]) < 0) return null;
  /* 이름 씻기 — 줄바꿈(머리 끼워 넣기)·따옴표·경로 글자를 걷고, 끝을 «열쇠의 확장자»로 맞춘다.
     ⚠ 이름이 파일 종류를 바꾸지 못하게 한다(「안내.pdf.exe」 꼴). */
  let n = String(이름 == null ? '' : 이름).replace(/[\u0000-\u001f\u007f"\\/:*?<>|]/g, '').trim();
  n = n.replace(/\.[A-Za-z0-9]{1,5}$/, '').slice(0, 100).trim() || '첨부';
  return { 자리: 'newsletter_files/' + m[1] + '.' + m[2], 이름: n + '.' + m[2] };
}
/* 내려받을 이름 머리 — 옛 브라우저는 filename= 을, 요즘 것은 filename*= 를 읽는다 */
function 내려받기머리(이름) {
  const 영 = String(이름 || 'file').replace(/[^\x20-\x7e]/g, '_').replace(/["\\]/g, '_');
  return 'attachment; filename="' + 영 + '"; filename*=UTF-8\'\'' + encodeURIComponent(String(이름 || 'file'));
}
...
module.exports = { 회차열쇠, 읽기, 볼수있나, 쪽, 없는쪽, 까닭말, 꼬리제목, 전문폭,
  그림열쇠, 쇼볼수있나, 쇼보안, 파일열쇠, 내려받기머리, 파일끝들 };

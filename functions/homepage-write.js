...
/* 글 «읽는» 쪽 — 관리자로 들어가 있으면 여기에 「관리」 차림표가 붙는다.
   고치는 화면에 「비공개」가 없으니, 내리는 길이 있다면 이 쪽에 이름이 보인다.
   ⚠ 읽기만 한다. 여기서 찾은 act 를 «짐작해서 부르지» 말 것 — 지우는 것이 섞여 있다. */
function 읽는주소(srl) {
  const n = Number(srl);
  if (!Number.isInteger(n) || n <= 0) return null;
...
/* ── 휴지통으로 내리기 ─────────────────────────────────────────────────
   ★ 2026-09-14 정찰로 확인한 것 — 이 게시판에는 「비공개」로 바꾸는 자리가 «없다».
     ...
     있는 것은 procDocumentManageCheckedDocument 하나이고, 그것이 하는 일은
     이동·복사·삭제·휴지통 넷이다.
       · 이동  — 갈 곳이 공지사항·파트너뿐이라 거기에 나타난다. 못 쓴다.
       · 삭제  — 되돌릴 수 없다. 대표 지시로 «절대» 안 쓴다.
       · 휴지통 — 홈페이지에서 사라지고, 글·사진·경력은 그대로 남고, 되살릴 수 있다.
     그래서 «휴지통»이 「지우지 말고 비공개」라는 지시에 가장 가깝다 (대표 승인 2026-09-14).

   ⚠⚠ 이 파일에서 나가는 type 은 «trash» 하나뿐이다. delete 를 절대 만들지 말 것 —
     한 글자 차이로 되살릴 수 없게 된다. 검사가 기계로 지킨다. */
const 내리는type = "trash";
const 절대안쓰는type = ["delete", "move", "copy"];

/* 문서 관리 명령을 보낼 몸통. 고를 글은 cart 에 담긴다(문서 관리 화면의 체크상자 이름). */
function 휴지통몸통(글번호들, 확인표) {
  const 번호 = (Array.isArray(글번호들) ? 글번호들 : [글번호들])
    .map((n) => Number(n))
    .filter((n) => Number.isInteger(n) && n > 0);
  if (!번호.length) return { ok: false, why: "내릴 글 번호가 없습니다" };
  const p = new URLSearchParams();
  p.append("module", "document");
  p.append("act", "procDocumentManageCheckedDocument");
  /* ⚠ 여기가 그 한 글자다. 상수에서만 온다 — 부르는 쪽이 정하게 두지 않는다. */
  p.append("type", 내리는type);
  번호.forEach((n) => p.append("cart", String(n)));
  if (확인표) p.append("_rx_csrf_token", String(확인표));
  return { ok: true, 몸통: p.toString(), 번호: 번호 };
}

function 문서관리보낼주소() { return ORIGIN + "/index.php"; }

/* ── 정찰 ──────────────────────────────────────────────────────────────
   고치는 화면에 «어떤 칸이 있는지»만 본다. 값은 한 글자도 안 본다.
   ★ 까닭 — 사진 넣기·비공개처럼 «우리가 아직 이름을 모르는 칸»을 다룰 때,
     짐작해서 만들면 엉뚱한 칸에 써서 사진이 날아간다. 먼저 보고 만든다.
   ⚠ 값(value·본문)은 절대 담지 않는다. 이 답은 화면에 뜨고 기록에도 남는다 —
     경력·이름·사진 주소가 거기 섞이면 안 된다. 고르개의 «보기 글자»만 예외로
     담는다(비공개/공개처럼 칸의 뜻을 말해 주는 딱지라서다). */
function 정찰(html) {
  const 본문 = String(html || "");
  const 글칸 = [], 파일칸 = [], 딸깍칸 = [], 숨은칸 = [], 고르개 = [], 넓은칸 = [];
  ...
  while ((m = ire.exec(본문))) {
    ...
    if (종류 === "file") 파일칸.push(이름);
    else if (종류 === "checkbox" || 종류 === "radio") 딸깍칸.push(이름 + "(" + 종류 + ")");
    else if (종류 === "hidden") 숨은칸.push(이름);
    else if (종류 !== "submit" && 종류 !== "button" && 종류 !== "image") 글칸.push(이름);
  }
  const tre = /<textarea\b([^>]*)>/gi;
  while ((m = tre.exec(본문))) {
    const 이름 = 속성("<t " + m[1] + ">", "name");
    if (이름) 넓은칸.push(이름);
  }
  ...
  while ((m = sre.exec(본문))) {
    ...
    const ore = /<option\b[^>]*>([\s\S]*?)<\/option>/gi;
    let o;
    while ((o = ore.exec(m[2]))) 글자들.push(이름다듬기(글자되돌리기(o[1])));
    고르개.push(이름 + "[" + 글자들.slice(0, 8).join("/") + "]");
  }

  /* 이름표 → 칸 이름 짝. 사람이 보는 이름으로 어느 칸인지 알 수 있게 한다. */
  const 이름표 = [];
  const rre = /<tr\b[^>]*>([\s\S]*?)<\/tr>/gi;
  while ((m = rre.exec(본문))) {
    const 줄 = m[1];
    ...
    const 보임 = 이름다듬기(머리[2]);
    const 이름 = 속성("<x " + 칸[2] + ">", "name");
    if (보임 && 이름) 이름표.push(보임 + " → " + 이름
      + ((속성("<x " + 칸[2] + ">", "type") || "").toLowerCase() === "file" ? " (파일)" : ""));
  }

  /* ★ 이 화면에서 «할 수 있는 일»의 이름들(act=…).
       2026-09-13 에 알게 된 것 — 구성원 게시판의 고치는 화면에는 「비공개」 칸이
       아예 없다(고르개는 is_notice, 딸깍은 title_bold 뿐). 그러면 퇴사자를 내리는
       길이 «어디»에 있는지 찾아야 하는데, 그 답이 이 목록에 있다.
     ⚠ 이름만 모은다. 부르지 않는다 — 짐작해서 부르면 글이 지워진다. */
  /* ⚠ 두 곳에 적힌다 — 링크 주소(act=…)와 숨은 칸(name="act" value="…").
       한쪽만 보면 «지금 이 화면이 무엇을 하는 화면인지»를 놓친다. */
  const 행위들 = [...new Set(
    [...본문.matchAll(/\bact=([A-Za-z][A-Za-z0-9_]{2,50})/g)].map(x => x[1])
      .concat([...본문.matchAll(/<input\b[^>]*>/gi)]
        .map(x => x[0])
        .filter(t => 속성(t, "name") === "act")
        .map(t => 속성(t, "value"))
        .filter(Boolean))
  )].sort();

  return { 글칸: 글칸, 넓은칸: 넓은칸, 파일칸: 파일칸, 딸깍칸: 딸깍칸,
           고르개: 고르개, 숨은칸: 숨은칸, 이름표: 이름표, 행위들: 행위들,
           번호값: 번호값들(본문),
...
function 번호값들(html) {
  ...
  while ((m = ire.exec(본문))) {
    const 이름 = 속성(m[0], "name");
    if (!이름 || !/srl(\[\])?$/i.test(이름)) continue;
    const 값 = 속성(m[0], "value");
    if (/^\d{1,12}$/.test(값)) (out[이름] = out[이름] || []).push(값);
  }
  Object.keys(out).forEach(k => { out[k] = [...new Set(out[k])].slice(0, 20); });
  return out;
}

...
exports.homepageWrite = functions
  ...
  .https.onRequest(async (req, res) => {
    ...
    try {
      ...
      if (방식 === "정찰") {
        const 본것 = HW.정찰(화면);
        /* ★ 글 «읽는» 쪽도 함께 본다 — 고치는 화면에 「비공개」가 없어서
             (2026-09-13 확인) 내리는 길이 어디 있는지 찾아야 한다.
             관리자로 들어가 있으면 읽는 쪽에 「관리」 차림표가 붙는다.
           ⚠ 읽기만 한다. 여기서 본 act 를 짐작해서 부르지 않는다. */
        let 읽는쪽 = null;
        try {
          const q = await 홈부르기(HW.읽는주소(몸.srl), 그릇);
          const q답 = await q.text();
          const 봄 = HW.정찰(q답);
          읽는쪽 = { 상태: q.status, 크기: 봄.크기, 행위들: 봄.행위들,
                     고르개: 봄.고르개, 딸깍칸: 봄.딸깍칸 };
        } catch (e) { 읽는쪽 = { 막힘: String((e && e.message) || e) }; }

        res.json(Object.assign({ ok: true, 방식: "정찰", srl: Number(몸.srl),
          저장됨: false, 걸음: 걸음, 읽는쪽: 읽는쪽 }, 본것));
        return;
      }
      ...
      /* ★ 2026-09-13 — 「비공개 자리를 못 찾았다」로 끝나면 대표께서 다시 누르실
           일이 생긴다. 그러지 않게, 못 찾은 그 자리에서 «이 게시판이 할 수 있는
           일»을 함께 실어 보낸다. 사람이 한 번 더 누를 까닭을 없앤다.
         ⚠ 이름만 모은다. 여기서 본 act 를 짐작해서 부르지 않는다. */
      if ((새것.못찾은것 || []).some((t) => /비공개/.test(String(t)))) {
        try {
          const q = await 홈부르기(HW.읽는주소(몸.srl), 그릇);
          const 봄 = HW.정찰(await q.text());
          const 쓰는 = HW.정찰(화면);
          /* ★ 숨은 칸 이름과 번호 값도 싣는다 (2026-09-14). 「할 수 있는 일」의 이름을 알아도
               글 번호·모듈 번호와 숨은 칸 이름이 없으면 그 요청을 «지을» 수 없다.
             ⚠ 값은 srl 로 끝나는 숫자만 — 그 밖의 값은 정찰 규칙대로 안 담는다. */
          알림.살펴본것 = { 쓰는화면: 쓰는.행위들, 쓰는화면숨은칸: 쓰는.숨은칸,
                            쓰는화면번호: 쓰는.번호값,
                            읽는화면: 봄.행위들, 읽는화면고르개: 봄.고르개,
                            읽는화면딸깍: 봄.딸깍칸, 읽는화면번호: 봄.번호값 };
          /* 관리자 «문서 관리» 화면까지 — 글 상태를 바꾸는 자리가 있다면 여기다 */
          try {
            const a = await 홈부르기(HW.문서관리주소(), 그릇);
            const 관리 = HW.정찰(await a.text());
            알림.살펴본것.문서관리 = 관리.행위들;
            알림.살펴본것.문서관리고르개 = 관리.고르개;
            알림.살펴본것.문서관리딸깍 = 관리.딸깍칸;
            알림.살펴본것.문서관리숨은칸 = 관리.숨은칸;
            알림.살펴본것.문서관리번호 = 관리.번호값;
            알림.살펴본것.문서관리상태 = a.status;
          } catch (e2) { 알림.살펴본것.문서관리 = ["(못 열었다: " + String((e2 && e2.message) || e2) + ")"]; }
        } catch (e) { 알림.살펴본것 = { 막힘: String((e && e.message) || e) }; }
      }
  ...
  .onRun(async () => {
    ...
    for (const 출처 of 지역뉴스부품.출처들) {
      ...
      try {
        ...
        읽은출처++;
        let 이출처새것 = 0;
        let 후보 = 지역뉴스부품.후보만들기(xml, 출처, Object.assign({}, 있던것, patch), Date.now());
        /* ★★ 「마감지남 거럼」 (대표 지시 2026-09-13).
             ⚠⚠ 실측: 천안시 기업지원 여덟 건이 «여덟 다 마감»이었다(5/18 · 6/10 · 6/26 ·
               7/31 · 8/21 · 8/31 · 9/11). 지자체 게시판은 지난 공고가 «첫 쪽에 그대로»
               쌓여 있어, 목록만 보고 담으면 끝난 것만 검토함에 찬다.
             ★ 목록에는 기간이 없다 — 상세를 열어야 안다. 그래서 기한확인 샘만 한 쪽씩 연다.
             ⚠ 상세를 못 읽으면 «버리지 않는다» — 못 읽은 것과 마감된 것은 다르다. */
        if (출처.기한확인 && 후보.length) {
          const 살아있는것 = [];
          let 마감뺀것 = 0;
          for (const x of 후보) {
            let 지났나 = false;
            try {
              const 상세 = await 글자로받기(x.링크);
              const 글 = String(상세).replace(/<[^>]+>/g, " ").replace(/&nbsp;/g, " ");
              지났나 = 지역뉴스부품.마감지났나(x.제목 + " " + 글, Date.now());
            } catch (e) { 지났나 = false; }
            if (지났나) { 마감뺀것++; continue; }
            살아있는것.push(x);
          }
          if (마감뺀것) console.log("[지역뉴스] " + 출처.id + " — 마감 지나 뺀 것 " + 마감뺀것 + "건");
          후보 = 살아있는것;
        }
        후보.forEach(function(x){ patch[x.id] = x; 새것++; 이출처새것++; });
    ...
    try {
      ...
      for (const m of 목록) {
        try {
          ...
          /* ★ 오늘을 넘겨 «마감 지난 지원사업»을 아예 안 만들게 한다
               (대표 지시 2026-09-13 「마감지남 거럼」). 기한이 없는 자료는 그대로 온다. */
          const it = 자료부품.자료로만들기(상세, { 목록제목: m.제목, 오늘: Date.now() });
